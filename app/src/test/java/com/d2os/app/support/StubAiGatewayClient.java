...
public class StubAiGatewayClient {
  ...
  public static final class LatencyControllableGateway implements AiGatewayClient {
    ...
    // index block with a dangling reference or a contradictory attribute) for the consistency
    // check.
    ...
    /**
     * Vector dimensionality of the deterministic test embeddings. Phase 2 (T004) sizes the {@code
     * knowledge_item.embedding vector(N)} column and T007's real {@code embed(...)} to this same
     * constant so stub-seeded fixtures and the pgvector column agree.
     */
    public static final int EMBEDDING_DIMENSIONS = 384;

    /**
     * Deterministic, provider-free embedding for integration tests (T003, research R3): the same
     * content always yields the same L2-normalized vector, so KnowledgeRetrievalIT,
     * KnowledgeReplayIT, and KnowledgeLeakageIT are reproducible with no model call and replay
     * stays byte-identical. Seeded from a SHA-256 of the content expanded through a pure LCG — no
     * {@code Math.random}, matching this class's "reproducible across replay" contract.
     *
     * <p>T007 adds {@code embed(EmbedRequest) → EmbedResult} to {@link AiGatewayClient}; its stub
     * override will delegate here (recording model id {@code stub-embed-1.0}). Kept as a standalone
     * helper for now so this Setup task carries no dependency on the Phase 2 gateway types.
     */
    ...
    /**
     * T007 override: deterministic, provider-free embedding delegating to {@link
     * #deterministicEmbedding(String)} — no model call, reproducible across replay.
     */
    @Override
