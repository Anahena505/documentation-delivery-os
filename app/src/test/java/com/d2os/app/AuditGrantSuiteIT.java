...
class AuditGrantSuiteIT {
  ...
  void appRoleCannotUpdateOrDeleteAuditStream() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      assertThrows(
          SQLException.class,
          () -> exec(conn, "UPDATE audit_entry SET action = 'tampered' WHERE 1 = 0"),
          "d2os_app must not be able to UPDATE audit_entry");
      assertThrows(
          SQLException.class,
          () -> exec(conn, "DELETE FROM audit_entry WHERE 1 = 0"),
          "d2os_app must not be able to DELETE from audit_entry");
      assertThrows(
          SQLException.class,
          () -> exec(conn, "UPDATE event_outbox SET event_type = 'tampered' WHERE 1 = 0"),
      ...
      assertThrows(
          SQLException.class,
          () -> exec(conn, "UPDATE progress_event SET kind = 'tampered' WHERE 1 = 0"),
          "d2os_app must not be able to UPDATE progress_event");
      assertThrows(
          SQLException.class,
          () -> exec(conn, "DELETE FROM progress_event WHERE 1 = 0"),
