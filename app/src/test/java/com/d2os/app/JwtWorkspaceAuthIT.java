 ...
 * JWT resolves its workspace from the token's {@code workspace_id} claim; an
 * invalid/tampered/expired token — or none at all — is rejected with 401, WITHOUT silently falling
 * back to a client-asserted header. The header fallback itself is exercised implicitly by every
 * other IT in this suite (it is enabled only via {@code
 * app/src/test/resources/application.properties}, T067's javadoc); this class additionally proves
 * the fallback is refused outright when disabled (the real-deployment posture), via a class-local
 * property override.
 */
...
class JwtWorkspaceAuthIT {
  ...
  void validJwtResolvesWorkspaceAndRequestSucceeds() {
    ...
    assertEquals(
        ...
        "a valid JWT must resolve the workspace and let the request through");
  }
  ...
  void expiredJwtIsRejected() throws InterruptedException {
    ...
    assertEquals(
        401,
        resp.getStatusCode().value(),
        "an expired token must be rejected even though correctly signed");
  }
  ...
  void missingAllCredentialsIsRejected() {
    ...
    assertEquals(
        401, resp.getStatusCode().value(), "a request with no credentials at all must be rejected");
  }
    ...
    void headerFallbackRefusedWhenDisabled() {
      ...
      ResponseEntity<Map> resp =
          rest.exchange(
              ...
              HttpMethod.POST,
              new HttpEntity<>(Map.of("formData", Map.of("category", "initiation")), h),
              Map.class);
      assertEquals(
          401,
          resp.getStatusCode().value(),
  ...
  private ResponseEntity<Map> submit(HttpHeaders headers) {
    ...
    return rest.exchange(
        ...
        HttpMethod.POST,
        new HttpEntity<>(Map.of("formData", Map.of("category", "initiation")), headers),
        Map.class);
  }
