...
/**
 * The deterministic sensitivity/PII pre-filter (T022, FR-010, research R7). Runs a fixed set of regex
 * detectors — EMAIL, PHONE, ID_NUMBER (IBAN / national-id-ish), CREDENTIAL (api-key / secret shapes) —
 * plus propagation of intake field-level sensitivity tags (T3-a): any caller-supplied sensitive term
 * found in the content is a {@code TAGGED_SENSITIVE} finding sourced {@code INTAKE_TAG:<field>}.
 *
 * <p>Default-deny (T3-c): every detected span is redacted OUT of the candidate's content before the
 * Curator ever sees it. The pre-filter therefore both records {@link PrefilterFinding} rows (the audit
 * trail of what was detected, at their spans in the ORIGINAL content) and rewrites the candidate content
 * with those spans replaced by a fixed {@code [REDACTED:<category>]} marker. The candidate transitions
 * CAPTURED → PREFILTERED in the same transaction.
 *
 * <p>Findings are append-only (V13 REVOKEs UPDATE/DELETE); the content rewrite is an allowed
 * {@code capture_candidate} UPDATE (status transitions + this default-exclusion edit on revision 1).
 */
@Service
public class SensitivityPreFilter {
    ...
    public SensitivityPreFilter(PrefilterFindingRepository findingRepository,
    ...
    /**
     * Run the pre-filter over a candidate's content: detect + record findings, redact detected spans out
     * of the content by default (T3-c), and transition the candidate CAPTURED → PREFILTERED.
     *
     * @param sensitiveTerms caller-supplied intake field-level sensitivity terms (T3-a). Each occurrence
     *                       in the content is recorded as a {@code TAGGED_SENSITIVE} finding and redacted.
    ...
    public List<PrefilterFinding> prefilter(UUID candidateId, List<String> sensitiveTerms) {
        ...
        if (sensitiveTerms != null) {
            for (String term : sensitiveTerms) {
                ...
                while ((idx = content.indexOf(term, idx)) >= 0) {
                    spans.add(new Span(idx, idx + term.length(),
                            PrefilterFinding.Category.TAGGED_SENSITIVE, "INTAKE_TAG:" + term));
                    idx += term.length();
                }
            }
        }
        ...
        for (Span s : spans) {
            PrefilterFinding finding = new PrefilterFinding(
                    UUID.randomUUID(), candidate.getWorkspaceId(), candidateId,
                    s.category(), s.start(), s.end(), s.source());
            findingRepository.save(finding);
            findings.add(finding);
        }

        // Redact detected spans out of the content by default (T3-c). Rewrites right-to-left so earlier
        // span offsets stay valid while later ones are replaced. Content + status change persist together
        // through the entity (one save) so a stale JPA write-back cannot clobber a separate content edit.
        ...
        promotionGateService.recordPrefilterPass(candidate,
                findings.size() + " finding(s) detected and redacted by default");
        return findings;
    }
