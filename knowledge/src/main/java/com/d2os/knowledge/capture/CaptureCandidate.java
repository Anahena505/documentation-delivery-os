...
/**
 * A lessons-learned proposal from the case-end capture subprocess (FR-008, V13, data-model.md). Born
 * PROJECT-scoped/confidential/non-promotable; the promotion pipeline is a strict, fixed-order state
 * machine with NO skip path:
 *
 * <pre>
 ...
 * {@code revisionOf} = the prior id) so the pre-redaction version is preserved for audit (FR-011). The
 * D4 gate reviews the LATEST revision only.
 *
 * <p>Every V13 {@code capture_candidate} column is mapped here; {@code tags} is a Postgres {@code text[]}
 * mapped as {@code String[]} with {@link JdbcTypeCode}({@link SqlTypes#ARRAY}) exactly as
 * {@code KnowledgeItem.tags} in US1. The status mutators VALIDATE the allowed transitions and throw
 * {@link IllegalCandidateTransitionException} on any illegal move — the state machine is enforced in
 * code, not merely commented.
 */
@Entity
@Table(name = "capture_candidate")
public class CaptureCandidate {

    public enum Status { CAPTURED, PREFILTERED, REDACTED, D4_PENDING, PUBLISHED, REJECTED }

    public enum RejectionStage { PREFILTER, CURATION, D4 }
     ...
     */
    public CaptureCandidate(UUID id, UUID workspaceId, UUID caseInstanceId, UUID projectId,
     ...
     */
    static CaptureCandidate redactionRevision(UUID id, CaptureCandidate prior, String title,
     ...
     * JPA write-back clobbering a separate raw-JDBC content update). This is the allowed
     * {@code capture_candidate} UPDATE (T3-c default exclusion on revision 1).
     */
    ...
    /**
     * Enforce the fixed-order state machine (data-model.md §CaptureCandidate). Any REDACTED, PREFILTERED,
     * CAPTURED or D4_PENDING state may transition to REJECTED; the forward path is strictly ordered.
     * Terminal states (PUBLISHED, REJECTED) accept no further transition.
     */
    private void transitionTo(Status target) {
        ...
        boolean allowed = switch (target) {
            ...
            case REJECTED    -> current == Status.CAPTURED || current == Status.PREFILTERED
                                || current == Status.REDACTED || current == Status.D4_PENDING;
    ...
    public UUID getRevisionOf() { return revisionOf; }
    public String getTitle() { return title; }
    public String getContent() { return content; }
    public String[] getTags() { return tags; }
    public String getStatus() { return status; }
    public UUID getCuratorOperationExecutionId() { return curatorOperationExecutionId; }
