 ...
 *
 * <ol>
 *   <li><b>Gate order</b> PREFILTER → CURATION → D4 — a gate cannot pass until its predecessors have
 *       (out-of-order → {@link GateOrderViolationException} → 409).</li>
 *   <li><b>At-most-one PASS per (candidate, gate)</b> — checked here (belt) and enforced by the V13
 *       {@code uq_promotion_gate_pass} partial unique index (braces).</li>
 *   <li><b>D4 is non-self-satisfiable</b> — the approver must hold the workspace-owner role AND differ
 *       from the redaction actor (→ {@link D4AuthorizationException} → 403).</li>
 *   <li><b>On D4 APPROVE</b> — publish the KnowledgeItem version via {@link EmbeddingIndexer#publish}
 *       with {@code source_candidate_id} provenance (scope raised to the approved level, default
 *       WORKSPACE), and transition the candidate → PUBLISHED.</li>
 *   <li><b>On any REJECT</b> — set status REJECTED + stage + reason, non-promotable, NO partial
 *       promotion.</li>
 *   <li>Every gate outcome writes a {@code promotion_gate_record}; every D4 outcome additionally writes
 *       a Decision + AuditEntry — all in ONE transaction.</li>
 * </ol>
 */
@Service
public class PromotionGateService {
    ...
    public PromotionGateService(CaptureCandidateRepository candidateRepository,
    ...
    // Gate records are keyed by the ROOT candidate id (revision 1) so a redaction revision's CURATION/D4
    ...
    public void recordPrefilterPass(CaptureCandidate candidate, String detail) {
        savePass(candidate, rootId(candidate), PromotionGateRecord.Gate.PREFILTER, "system:prefilter", null, detail);
    }

    // ---- CURATION gate ---------------------------------------------------------------------------

    /**
     * Record the CURATION PASS (second gate) with the redaction actor. The D4 gate reads this actor back
     * to enforce non-self-satisfiability. Requires the PREFILTER gate to have PASSed already.
     */
    @Transactional
     ...
     * @param isWorkspaceOwner whether the approver holds the workspace-owner role (authorization input
     *                      resolved by the caller; the gate requires it to be true)
     ...
     */
    @Transactional
    public UUID decideD4(UUID candidateId, String approver, boolean isWorkspaceOwner, boolean approve,
                         KnowledgeScope scopeLevel, String reason) {
        CaptureCandidate candidate = candidateRepository.findById(candidateId)
        ...
        // Non-self-satisfiable D4 (Q5): approver must be the workspace owner AND not the redaction actor.
        if (!isWorkspaceOwner) {
            throw new D4AuthorizationException("D4 approver " + approver + " lacks the workspace-owner role");
        }
        ...
        rejectAtGate(candidate, PromotionGateRecord.Gate.D4, CaptureCandidate.RejectionStage.D4,
                approver, reason == null ? "rejected at D4" : reason);
        return null;
    }

    /**
     * Reject the candidate at an earlier gate (PREFILTER or CURATION). Sets status REJECTED with the
     * stage + reason, records a {@code REJECT} gate row and an AuditEntry — no partial promotion. Used by
     * the pre-filter / curation steps when a gate fails.
     */
    @Transactional
    public void rejectAtGate(UUID candidateId, PromotionGateRecord.Gate gate,
                             CaptureCandidate.RejectionStage stage, String actor, String reason) {
        CaptureCandidate candidate = candidateRepository.findById(candidateId)
                .orElseThrow(() -> new NoSuchElementException("candidate " + candidateId));
    ...
    private UUID approveAndPublish(CaptureCandidate candidate, String approver,
                                   KnowledgeScope scopeLevel, String rationale) {
        ...
        UUID scopeRef = scope == KnowledgeScope.PROJECT ? candidate.getProjectId() : candidate.getWorkspaceId();

        UUID itemId = embeddingIndexer.publish(
                candidate.getWorkspaceId(), UUID.randomUUID(), knowledgeKey(candidate), 1,
                scope, scopeRef, List.of(candidate.getTags()), "en",
        ...
        decisionRepository.save(new DecisionRecord(
                decisionId, candidate.getWorkspaceId(), candidate.getCaseInstanceId(), "D4",
    ...
    private void rejectAtGate(CaptureCandidate candidate, PromotionGateRecord.Gate gate,
                              CaptureCandidate.RejectionStage stage, String actor, String reason) {
        ...
        if (gate == PromotionGateRecord.Gate.D4) {
            ...
            decisionRepository.save(new DecisionRecord(
                    decisionId, candidate.getWorkspaceId(), candidate.getCaseInstanceId(), "D4",
                    actor, reason, candidate.getId().toString()));
        }
        // REJECT rows are keyed by root id too, keeping the whole trail on one candidate chain.
        gateRepository.save(new PromotionGateRecord(
                UUID.randomUUID(), candidate.getWorkspaceId(), rootId(candidate),
                gate, PromotionGateRecord.Outcome.REJECT, decisionId, actor, reason));
        auditWriter.record(candidate.getWorkspaceId(), "capture_candidate", candidate.getId(),
                "REJECTED", actor, Map.of("gate", gate.name(), "stage", stage.name(), "reason", safe(reason)));
    }

    private void savePass(CaptureCandidate candidate, UUID candidateKey, PromotionGateRecord.Gate gate,
                          String actor, UUID decisionId, String detail) {
        ...
        try {
            gateRepository.save(new PromotionGateRecord(
                    UUID.randomUUID(), candidate.getWorkspaceId(), candidateKey,
                    gate, PromotionGateRecord.Outcome.PASS, decisionId, actor, detail));
    ...
    private String curationActor(UUID candidateId) {
        return gateRepository.findByCandidateIdOrderByCreatedAtAsc(candidateId).stream()
                .filter(r -> PromotionGateRecord.Gate.CURATION.name().equals(r.getGate())
