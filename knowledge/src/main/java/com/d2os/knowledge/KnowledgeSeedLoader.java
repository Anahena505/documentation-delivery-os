 ...
 * FR-021, Principle I). Lives in the {@code knowledge} module (not {@code CatalogSeedLoader})
 * because publishing an item goes through {@link EmbeddingIndexer} — and {@code catalog} cannot
 * depend on {@code knowledge} (the dependency is one-way: knowledge → catalog).
 *
 * <p>Items seed into the reserved system-global workspace, whose {@code knowledge_item} partition
 * V13 already created — so no partition provisioning is needed here. Idempotent by {@code (key,
 * version)}: re-running the app must not duplicate a seed item (the V13 {@code
 * uq_knowledge_item_key_version} would otherwise reject it). Runs after {@link
 * com.d2os.catalog.CatalogSeedLoader} (default order) since it needs no catalog rows, but ordered
 * late defensively.
 */
@Component
@Order(100)
public class KnowledgeSeedLoader implements ApplicationRunner {
   ...
   * soft reference (no FK, V13), so this project-scoped fixture is publishable at bootstrap before
   * any real project exists; it is only ever retrieved by a query that passes exactly this
   * projectId — which also makes it a standing fixture proving project-scope filtering (a query
   * with a different or null projectId must never see it).
   */
  public static final UUID DEMO_PROJECT_REF =
