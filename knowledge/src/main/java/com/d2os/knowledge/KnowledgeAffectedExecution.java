...
/**
 * A deprecation flag on a past {@code operation_execution} whose injection snapshot referenced a
 * now- deprecated KnowledgeItem version (US3, FR-015, V13 {@code knowledge_affected_execution}). It
 * records that history <em>referenced</em> the retired item — it NEVER mutates the flagged
 * execution, its snapshot, or its output, so replay still reproduces the original byte-for-byte
 * (FR-016, SC-007).
 *
 * <p>Append-only except the single {@link #markReviewed()} acknowledgement flip (V13 REVOKEs DELETE
 * but permits the {@code review_status} UPDATE). Rows are inserted by {@link DeprecationService}
 * via an insert-select over {@code knowledge_injection_snapshot}, so this entity is otherwise
 * read/flip only.
 */
@Entity
