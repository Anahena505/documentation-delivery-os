...
/**
 * The knowledge-influence surface (US4, T034, contracts/api.yaml): trigger a paired influence
 * evaluation and read the resulting KPI. The read NEVER fabricates — an item that was never
 * evaluated returns {@code NOT_YET_MEASURABLE} with no value (FR-018).
 */
@RestController
public class InfluenceController {

  // dimensions->>'version' extracts the JSON number as text, so the (key,version) filter binds
  // version
  ...
  public InfluenceController(
      InfluenceEvaluationService influenceEvaluationService,
   ...
   * rubric for the given persona. Returns 202 Accepted with the measured result (the evaluation
   * runs synchronously against the deterministic gateway; 202 reflects that this is a diagnostic
   * side-run, not a delivery operation). 404 if the item or case is unknown.
   */
  ...
  /**
   * Read an item version's influence KPI. {@code MEASURED} with the sample series + latest delta
   * when at least one paired evaluation has run; {@code NOT_YET_MEASURABLE} (no value) for a
   * never-evaluated item — never fabricated (FR-018). Accepts either {@code itemId} or explicit
   * {@code key}+{@code version}.
   */
  @GetMapping("/api/v1/metrics/knowledge-influence")
  public ResponseEntity<InfluenceMetric> metric(
    ...
    if (samples.isEmpty()) {
      return ResponseEntity.ok(
          new InfluenceMetric("NOT_YET_MEASURABLE", resolvedKey, resolvedVersion, null, List.of()));
    }
    double latestDelta = samples.get(samples.size() - 1).value();
    return ResponseEntity.ok(
        new InfluenceMetric("MEASURED", resolvedKey, resolvedVersion, latestDelta, samples));
  }
  ...
  public record InfluenceMetric(
      String status, String key, int version, Double latestDelta, List<Sample> samples) {}
}
