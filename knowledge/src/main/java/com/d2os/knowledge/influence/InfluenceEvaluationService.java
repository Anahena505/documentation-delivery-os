 ...
 * (US4, T032, FR-017, research R9). Against a chosen case + persona (whose pinned rubric supplies
 * the scoring criteria), it runs the persona step twice under the SAME rubric version:
 *
 * <ol>
 *   <li><b>with</b> — the target item force-included as the sole injected knowledge;
 *   <li><b>without</b> — no injected knowledge.
 * </ol>
 *
 * The influence value is the measured {@code withScore - withoutScore} rubric-score delta, emitted
 * on the existing {@code kpi_sample} stream tagged with the item's {@code (key, version)}. Both
 * runs are recorded with {@code evaluation=true} so they never feed delivery (and, being
 * operation_execution rows with no produced artifact, cannot reach a package regardless).
 *
 * <p><b>Why not go through {@code PersonaExecutionService}?</b> That path auto-retrieves knowledge
 * from the persona profile and drives the bounded revise loop — it offers no way to force a
 * specific item in or out, nor to flag the run as an evaluation. This service therefore reuses the
 * same building blocks (envelope builder, prompt renderer, gateway, validation pipeline, recorder)
 * directly for a single controlled generation per side. It intentionally does NOT charge the case
 * token budget (an evaluation is diagnostic and must not deny a live case its budget).
 */
@Service
public class InfluenceEvaluationService {
  ...
  public InfluenceEvaluationService(
  ...
  public record InfluenceResult(
      ...
      double withoutScore,
      double delta,
      UUID withExecutionId,
      UUID withoutExecutionId) {}

  /**
   * Run the paired influence evaluation of {@code itemId} against {@code caseId}'s pinned rubric
   * for {@code personaKey}, and emit the delta.
   *
  ...
  public InfluenceResult evaluate(UUID itemId, UUID caseId, String personaKey) {
    ...
    KnowledgeProvider.InjectedItem target =
    ...
    kpiEmitter.emitKnowledgeInfluence(
        kase.getWorkspaceId(), caseId, delta, item.getKey(), item.getVersion());
  ...
  private RunResult runOnce(CaseInstance kase, String personaKey, PersonaEnvelope env) {
    ...
    ValidationResult validation =
        validationPipeline.validate(result.outputText(), env.rubricJson());
    ...
    PersonaInvocation invocation =
        new PersonaInvocation(
            ...
            kase.getWorkspaceId(),
            kase.getId(),
            env.personaDefinitionId(),
            env.personaDefinitionVersion(),
            nextSequenceNo(kase.getId()));
   ...
   */
  private PersonaEnvelope variant(
      PersonaEnvelope base, List<KnowledgeProvider.InjectedItem> injected) {
    int tokens =
        injected.stream().mapToInt(i -> i.content() == null ? 0 : i.content().length() / 4).sum();
    return new PersonaEnvelope(
        base.caseId(),
        base.personaKey(),
        base.personaDefinitionId(),
        base.personaDefinitionVersion(),
        base.promptDefinitionId(),
        base.promptDefinitionVersion(),
        base.promptTemplate(),
        base.rubricDefinitionId(),
        base.rubricDefinitionVersion(),
        base.rubricJson(),
        base.submissionFormDataJson(),
        injected,
        tokens,
