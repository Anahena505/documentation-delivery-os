...
/**
 * Retires KnowledgeItem versions and flags — never rewrites — the history that referenced them
 * (US3, T028, FR-014/015/016, research R8). In ONE transaction {@link #deprecate} does exactly two
 * things:
 *
 * <ol>
 *   <li><b>Retire</b> every currently-PUBLISHED item in the {@code [fromVersion, toVersion]} range
 *       for the key: flip {@code status → DEPRECATED} (append-only — content/hash untouched, so any
 *       snapshot that pinned the version still replays byte-identically, FR-016) and write one
 *       immutable {@code AuditEntry} per retired version.
 *   <li><b>Flag</b> the affected history: an insert-select over {@code
 *       knowledge_injection_snapshot} inserts one {@code knowledge_affected_execution} row per
 *       distinct {@code operation_execution} whose snapshot referenced a retired version — reaching
 *       the owning case through {@code persona_invocation}. The snapshots and outputs are read-only
 *       here; they are NEVER modified (Principle III, FR-016).
 * </ol>
 *
 ...
 *
 * <p>New-operation retrieval already excludes DEPRECATED items — {@link
 * KnowledgeRetrievalService}'s query filters {@code status = 'PUBLISHED'} (T029) — while in-flight
 * snapshotted envelopes, being decoupled soft references, are unaffected.
 */
@Service
public class DeprecationService {
  ...
  /**
   * The outcome of a deprecation: how many versions were retired and how many executions were
   * flagged.
   */
   ...
   * {@code review_status} OPEN → REVIEWED and audit the acknowledgement, in ONE transaction. Only
   * the flag row is touched — the flagged {@code operation_execution}, its snapshot, and its output
   * are never modified (FR-016). Idempotent-safe: re-reviewing an already-REVIEWED flag simply
   * re-audits.
   *
   * @throws NoSuchElementException the flag id does not exist (or is outside the caller's
   *     workspace)
   */
  @Transactional
  public KnowledgeAffectedExecution reviewAffectedExecution(UUID flagId, String actor) {
    KnowledgeAffectedExecution flag =
        affectedExecutionRepository
            .findById(flagId)
    ...
    auditWriter.record(
        flag.getWorkspaceId(),
        "knowledge_affected_execution",
        flag.getId(),
        "AFFECTED_EXECUTION_REVIEWED",
        actor,
        Map.of(
            "operationExecutionId",
            flag.getOperationExecutionId().toString(),
            "knowledgeItemKey",
            flag.getKnowledgeItemKey(),
            "knowledgeItemVersion",
            flag.getKnowledgeItemVersion()));
    return flag;
  }
  ...
  public DeprecationResult deprecateItem(UUID itemId, String reason, String actor) {
    ...
    return deprecate(
        item.getWorkspaceId(), item.getKey(), item.getVersion(), item.getVersion(), reason, actor);
  }

  /**
   * Deprecate every currently-PUBLISHED version of {@code key} in {@code [fromVersion, toVersion]}
   * and flag the executions that injected any retired version. Retiring an already-DEPRECATED-only
   * range is a no-op that flags nothing (versions are skipped, no rows inserted).
   *
   * @return the retired item ids and the count of newly-flagged executions
   */
  @Transactional
  public DeprecationResult deprecate(
      UUID workspaceId, String key, int fromVersion, int toVersion, String reason, String actor) {
    List<KnowledgeItem> versions =
        itemRepository.findByWorkspaceIdAndKeyOrderByVersionDesc(workspaceId, key);
    List<UUID> retired =
        versions.stream()
            ...
            .map(
                item -> {
                  ...
                  // Immutable audit of the governance action (FR-016) — the deprecation's system of
                  // record.
                  auditWriter.record(
                      workspaceId,
                      "knowledge_item",
