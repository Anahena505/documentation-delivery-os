...
public class CaseController {
  ...
  public ResponseEntity<CaseResponse> create(@Valid @RequestBody CreateCaseRequest request) {
    SubmissionLookup.SubmissionInfo submission =
        submissionLookup
            .find(request.submissionId())
            .orElseThrow(
                () ->
                    new CaseCreationException(
                        "submission " + request.submissionId() + " not found"));
   ...
   * superseded}/{@code deprecated} status surfaced (T025, US3, research R4). 404 when the case
   * isn't an Enhancement case, or its {@code BaselineResolutionDelegate} step hasn't run yet (no
   * {@code BASELINE_RESOLVED} audit entry) — both map through the same {@link
   * NoSuchElementException} → 404 path {@link CaseExceptionHandler} already provides.
   */
  ...
  public ResponseEntity<BaselineResponse> baseline(@PathVariable UUID id) {
    ...
    AuditEntryRecord entry =
        auditEntryRepository
            ...
            .orElseThrow(
                () -> new NoSuchElementException("case " + id + " has no resolved baseline"));
  ...
  public ResponseEntity<List<Map<String, Object>>> requiredArtifacts(@PathVariable UUID id) {
    ...
    CaseDefinitionSnapshot snapshot =
        snapshotRepository
            .findByCaseInstanceId(id)
            .orElseThrow(
                () -> new NoSuchElementException("case " + id + " has no pinned snapshot"));

    Set<String> producedKinds =
        Set.copyOf(
    ...
    List<Map<String, Object>> result =
        caseService.requiredArtifacts(snapshot).stream()
            .map(
                entry -> {
