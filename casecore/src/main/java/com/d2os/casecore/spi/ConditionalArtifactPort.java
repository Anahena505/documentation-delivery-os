...
/**
 * Port that lets casecore evaluate the conditional-artifacts DMN without depending on a
 * Flowable-DMN engine dependency itself (Phase 4 US5, T030-T032, research R6, FR-014/015) — same
 * dependency-inversion seam as {@link SubmissionLookup} (intake implements this too, since it
 * already carries the flowable-spring-boot-starter-dmn dependency for {@code
...
public interface ConditionalArtifactPort {
   ...
   * pinned {@link com.d2os.casecore.CaseDefinitionSnapshot}'s expected set must additionally
   * require.
   */
