 ...
 * so a long AI call — which produces no natural boundary events — never makes a Case look frozen.
 * The interval is set below the 5 s bound (default 4 s). Reads the in-memory {@link
 * ActiveCaseRegistry} rather than scanning the DB, so it needs no cross-tenant query; each write
 * goes through {@link HeartbeatWriter} to get a transaction with the Case's workspace RLS context
 * bound.
 */
@Component
public class ProgressHeartbeat {
  ...
  // US3 (T034): deliberately NOT @SchedulerLock'd. Unlike the other 8 scheduled jobs — each a
  // single
  // global sweep over every workspace that must run once per cycle cluster-wide — this beat reads
  // the
  // in-memory ActiveCaseRegistry, which holds only the Running cases hosted on THIS JVM. It is thus
  // naturally partitioned per instance: each node must beat its own cases every cycle. A single
  // global
  // ShedLock name would let just one instance beat per window, starving cases on every other
  // instance
  // of heartbeats and making them look frozen — the exact failure this job exists to prevent. A
  // true
  // per-case lock cannot be expressed via the static @SchedulerLock annotation (its name is a
  // compile-time constant) and would add no value here, since two instances never host the same
  // case's
  // in-memory registry entry. So no lock is correct (data-model.md §1: heartbeat is
  // per-case/partitioned).
  @Scheduled(
      fixedDelayString = "${d2os.casecore.progress.heartbeat-interval-ms:4000}",
