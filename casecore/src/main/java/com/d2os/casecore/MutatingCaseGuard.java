...
public class MutatingCaseGuard {
  ...
  /**
   * Acquire the Feature's single mutating-case slot for {@code caseId}. Guarded UPDATE: {@code SET
   * active_mutating_case_id=:caseId, aggregate_version=aggregate_version+1 WHERE id=:featureId AND
   * aggregate_version=:expectedVersion AND active_mutating_case_id IS NULL}. Zero rows updated
   * means either the version was stale or the slot was already occupied — either way this is a
   * conflict (never a queue, never a retry): throws {@link MutatingGuardConflictException} carrying
   * the Feature id and (best-effort, informational only) the case currently holding the slot.
   */
  @Transactional
