 ...
 * = SHA-256 over the ordered canonical serialization of the range, chained to the prior segment's
 * own hash ({@code genesis = 64 * '0'}). A workspace with zero new entries since its last seal is
...
public class AuditChainSealer {
  ...
  /**
   * Seal one workspace's unsealed tail. Returns the new segment, or {@code null} if nothing to
   * seal.
   */
  @Transactional
  public AuditChainSegment sealWorkspace(UUID workspaceId) {
    ...
    try {
      ...
      AuditChainSegment prior =
          segmentRepository.findFirstByWorkspaceIdOrderBySegmentSeqDesc(workspaceId).orElse(null);
      List<AuditEntryRecord> unsealed =
          prior == null
      ...
      AuditChainSegment segment =
          new AuditChainSegment(
              UUID.randomUUID(),
              workspaceId,
              nextSeq,
              unsealed.get(0).getId(),
              unsealed.get(unsealed.size() - 1).getId(),
              unsealed.size(),
              segmentHash,
              prevHash);
  ...
  private java.time.OffsetDateTime entryTxTime(UUID entryId) {
    return auditEntryRepository
        ...
        .orElseThrow(
            () -> new IllegalStateException("sealed audit_entry " + entryId + " no longer exists"));
  }
}
