 ...
 * stream (V21 {@code audit_chain_segment}, Phase 7 US5, T6-b, research R5). Per-workspace hash chain:
 * {@code segmentHash} covers this range's canonical serialization; {@code prevSegmentHash} chains to
 * the prior segment (genesis = 64 zero characters). Append-only except {@code lastVerifiedAt}, which
 * {@link AuditChainVerifier} updates.
 */
@Entity
...
public class AuditChainSegment {
    ...
    public AuditChainSegment(UUID id, UUID workspaceId, long segmentSeq, UUID fromEntryId, UUID toEntryId,
                             int entryCount, String segmentHash, String prevSegmentHash) {
        this.id = id;
    ...
    public UUID getFromEntryId() { return fromEntryId; }
    public UUID getToEntryId() { return toEntryId; }
    public int getEntryCount() { return entryCount; }
    public String getSegmentHash() { return segmentHash; }
    public String getPrevSegmentHash() { return prevSegmentHash; }
    public OffsetDateTime getSealedAt() { return sealedAt; }
