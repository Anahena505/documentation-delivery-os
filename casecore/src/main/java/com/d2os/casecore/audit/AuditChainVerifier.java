...
/**
 * Recomputes and checks the audit hash chain, on demand and on schedule (Phase 7 US5, T039,
 * research R5, FR-013). A mismatch — an altered or deleted entry inside an already-sealed range
 * recomputing to a different hash than what was sealed, or a broken {@code prevSegmentHash} link —
 * is a tamper alert: a {@code TAMPER_ALERT} in-app notification plus an outbox event, both via the
 * SAME {@code audit_entry} + {@code event_outbox} mechanism every other event in this codebase uses
 * (raw JDBC write to {@code in_app_notification}/{@code event_outbox} — {@code casecore} cannot
 * depend on {@code governance}, which owns those tables' JPA entities; same cross-module-raw-write
 * convention {@code ArtifactService}/{@code ConsistencyService} already establish for {@code
 * trace_link}).
 */
@Component
public class AuditChainVerifier {
  ...
  public record SegmentResult(
      long segmentSeq, boolean intact, String expectedHash, String recomputedHash) {}
  ...
  public AuditChainVerifier(
      ...
      AuditChainSegmentRepository segmentRepository,
  ...
  public ChainResult verifyWorkspace(UUID workspaceId) {
    ...
    try {
      ...
      List<AuditChainSegment> segments =
          segmentRepository.findByWorkspaceIdOrderBySegmentSeqAsc(workspaceId);
      ...
      for (AuditChainSegment segment : segments) {
        ...
        results.add(
            new SegmentResult(
                segment.getSegmentSeq(), intact, segment.getSegmentHash(), recomputed));
        ...
        expectedPrevHash =
            segment.getSegmentHash(); // chain continues on the RECORDED hash either way
      }
  ...
  private List<AuditEntryRecord> entriesInRange(UUID workspaceId, AuditChainSegment segment) {
    OffsetDateTime fromTime =
        auditEntryRepository
            .findById(segment.getFromEntryId())
            .map(AuditEntryRecord::getTxTime)
            .orElse(null);
    OffsetDateTime toTime =
        auditEntryRepository
            .findById(segment.getToEntryId())
            .map(AuditEntryRecord::getTxTime)
            .orElse(null);
    if (fromTime == null || toTime == null) {
      return List
          .of(); // a sealed entry no longer exists at all — recomputes empty, so hash mismatches
    }
  ...
  private void raiseTamperAlert(UUID workspaceId, AuditChainSegment segment) {
    ...
    jdbcTemplate.update(
        ...
        "Audit chain segment "
            + segment.getSegmentSeq()
            + " failed verification — possible tampering");
    jdbcTemplate.update(
