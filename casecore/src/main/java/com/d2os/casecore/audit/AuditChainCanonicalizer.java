...
final class AuditChainCanonicalizer {
   ...
   * seals/hashes remain valid. A non-null actor (OIDC mode) appends {@code |userId|role}, which
   * both changes the bytes (tamper-evidence for the actor) and keeps legacy rows unaffected.
   */
  static String canonicalize(List<AuditEntryRecord> entries) {
    ...
    for (AuditEntryRecord e : entries) {
      sb.append(e.getId())
          .append('|')
          .append(e.getWorkspaceId())
          .append('|')
          .append(e.getSubjectType())
          .append('|')
          .append(e.getSubjectId())
          .append('|')
          .append(e.getAction())
          .append('|')
          .append(e.getActor())
      ...
      if (actorUserId != null || actorRole != null) {
        sb.append('|').append(nullToEmpty(actorUserId)).append('|').append(nullToEmpty(actorRole));
      }
      sb.append('\n');
    }
    return sb.toString();
