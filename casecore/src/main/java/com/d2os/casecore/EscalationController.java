 ...
 * and records the gate comment as a Decision; fully re-driving the Flowable job from the failed
 * step is future work (the state transition + decision record are real now).
 */
@RestController
...
public class EscalationController {
  ...
  public ResponseEntity<List<EscalationView>> list(@PathVariable UUID caseId) {
    ...
    return ResponseEntity.ok(
        List.of(new EscalationView(caseId, "case-level escalation: revise loop exhausted")));
  }
  ...
  public ResponseEntity<Void> resolve(
    ...
    switch (request.resolution()) {
      case "regenerate_with_comment" ->
          caseService.resume(caseId, "human:" + safe(request.gateComment()));
