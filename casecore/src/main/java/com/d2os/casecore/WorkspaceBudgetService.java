 ...
 * cannot collectively evade the ceiling. A {@code token_cap} of 0 means unlimited (the default), so
 * a workspace only enforces a ceiling once one is set.
 */
@Service
public class WorkspaceBudgetService {
  ...
  public boolean wouldExceed(UUID workspaceId, long estimatedTokens) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
            "SELECT token_cap, tokens_consumed FROM workspace_budget WHERE workspace_id = ?",
            workspaceId);
  ...
  public void setCap(UUID workspaceId, long tokenCap) {
    ensureRow(workspaceId);
    jdbcTemplate.update(
        "UPDATE workspace_budget SET token_cap = ? WHERE workspace_id = ?", tokenCap, workspaceId);
  }
    ...
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
            "SELECT period_start, token_cap, tokens_consumed, rate_limit_per_minute "
                + "FROM workspace_budget WHERE workspace_id = ?",
            workspaceId);
    if (rows.isEmpty()) {
  ...
  public record BudgetView(
      String periodStart, long tokenCap, long tokensConsumed, int rateLimitPerMinute) {}
}
