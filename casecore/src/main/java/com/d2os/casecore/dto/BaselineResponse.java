...
public record BaselineResponse(
    UUID caseId, UUID featureId, UUID baselineCaseId, List<BaselineEntry> entries) {

  /**
   * One pinned baseline ArtifactRevision.
   *
   * <p><b>{@code superseded} (research R4/T025 design decision):</b> there is no existing concept
   * of "superseded" for an ArtifactRevision anywhere in this codebase — {@code superseded}/{@code
   * deprecated} exist only for {@code KnowledgeItem}, a completely different subsystem. Defined
   * here, simply and derivably from existing data with no new schema: a pinned baseline revision is
   * {@code superseded} when a STRICTLY NEWER revision now exists on the same Artifact (i.e. {@code
  ...
  private static boolean isSuperseded(
      JdbcTemplate jdbcTemplate, UUID artifactId, int pinnedRevisionNo) {
    Integer maxRevisionNo =
        jdbcTemplate.queryForObject(
            "SELECT max(revision_no) FROM artifact_revision WHERE artifact_id = ?",
            Integer.class,
            artifactId);
