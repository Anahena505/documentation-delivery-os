 ...
 * <ul>
 *   <li>{@code d2os.job.executions{job}} — a counter incremented once per run;</li>
 *   <li>{@code d2os.job.duration{job}} — a timer of each run's wall-clock latency;</li>
 *   <li>{@code d2os.job.failures{job}} — a counter incremented when a run throws.</li>
 * </ul>
 *
 * <p>Kept as a thin, constructor-injected write API (mirroring {@link KpiEmitter} in this module) so
 * any scheduled job can time itself without knowing about Micrometer's meter shapes. Unit-instantiable
 * with a {@code SimpleMeterRegistry} — no Spring context required.
 *
 * <p>Jobs in modules that already depend on {@code observability} (orchestration) inject this bean
 * directly; jobs in modules that cannot depend on {@code observability} without forming a Gradle cycle
 * (casecore, tenancy — {@code observability} depends on {@code casecore}) record the identical meters
 * inline against an injected {@code MeterRegistry} instead (T019).
 */
@Component
