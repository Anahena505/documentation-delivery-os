...
/**
 * Records the KPIs (T054, FR-015, §KP; Phase 3 US4 adds {@code knowledge_influence}). Emit points
 * live in the pipeline: {@code rubric_first_pass_rate} from persona validation, {@code
 * package_completeness} and {@code case_cost_tokens} from package assembly, and {@code
 * knowledge_influence} from the paired influence evaluation. Kept as a thin write API so any stage
...
public class KpiEmitter {
   ...
   * version contributed, attributed via {@code dimensions = {"key":..., "version":...}} on the
   * reused V9 {@code kpi_sample} stream. The delta is the measured with-minus-without value — never
   * fabricated.
   */
  @Transactional
  public void emitKnowledgeInfluence(
      UUID workspaceId, UUID caseId, double delta, String key, int version) {
