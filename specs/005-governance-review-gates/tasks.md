...
- [X] T044 [US6] Add PostgreSQL WAL-archiving DR configuration to the compose stack (`archive_command` to the MinIO/object-store bucket + nightly base backup for point-in-time recovery, RPO ≤ 15 min) in `docker-compose.yml` / compose overrides (research R7, NFR-8). **Deviation**: `archive_command` copies to a dedicated `d2os-wal-archive` Docker volume rather than the MinIO bucket — Postgres's `archive_command` shells out to a local filesystem path inside the container; archiving straight to S3-compatible object storage needs a tool like `wal-g`/`pgbackrest` (a new external dependency), which is a reasonable follow-up but out of scope for landing the core WAL-archiving mechanism itself. `archive_timeout=300` bounds RPO to 15 min even under idle load.
- [X] T045 [US6] Author the `ops/dr-drill.md` runbook (base restore + WAL replay to a timestamp; boot app on the restored DB; run the replay-audit + chain-verify smoke suites; measure wall-clock RTO ≤ 1 h) (research R7, NFR-8, SC-007)
- [X] T046 [US6] Execute the drill and record achieved RPO/RTO, date, and operator in `specs/005-governance-review-gates/quickstart-results.md` (shortfalls recorded as findings, not passes) (research R7, SC-007). **Executed, scoped**: this sandbox has no persistent Docker Compose stack, but PostgreSQL 16 is installed locally outside Docker — used it to run the real WAL-archiving → `pg_basebackup` → `recovery.signal`/`restore_command`/`recovery_target_time` point-in-time-restore mechanism end to end against a standalone cluster (not the full app stack/D2OS schema, which needs Docker Compose — out of reach here). Real measured RTO = 81.5s (extract + replay 7 WAL segments + ready) at drill scale; point-in-time boundary exact (80/80 pre-target rows recovered, 0/15 post-target rows leaked). One sub-check (idle-period `archive_timeout` auto-switch, as opposed to the forced-switch path the restore itself exercised and which worked correctly every time) was inconclusive within the observation window — flagged as unconfirmed, not failed. Full write-up, scope caveats, and next steps in `quickstart-results.md`.
