...
- [X] T001 Register the `knowledge` module in `settings.gradle`.
...
- [X] T002 Configure `knowledge/build.gradle` with the correct acyclic dependencies.
...
- [X] T003 [P] Add knowledge config keys under `d2os.knowledge` in `app/src/main/resources/application.yml`.
...
- [X] T004 [P] Ensure the `knowledge` module is component-scanned by the app.
...
- [X] T005 Create/verify the knowledge core migration.
...
- [X] T006 Verify the remaining V13 tables + RLS + append-only REVOKEs in the same file.
...
- [X] T007 Verify the per-workspace partition-creation function (R2/T2-b).
...
- [X] T008 Wire partition creation into workspace provisioning (same transaction as the workspace row).
...
- [X] T009 Create/verify the injection-snapshot migration.
...
- [X] T010 [P] Verify the `KnowledgeItem` entity + repository.
...
- [X] T011 [P] Verify the `KnowledgeScope` scope-lattice value type.
...
- [X] T012 Verify the `KnowledgeProvider` SPI in persona (the acyclic seam).
...
- [X] T013 Verify the AI Gateway embeddings operation.
...
- [X] T014 [P] Verify the deterministic stub embedding (provider-free ITs).
...
- [X] T015 Verify the gateway workspace-scope assertion seam (T2-c/R5, second isolation layer).
...
- [X] T016 [P] [US1] Verify `KnowledgeRetrievalIT` (SC-001/002).
...
- [X] T017 [P] [US1] Verify `KnowledgeReplayIT` (SC-003).
...
- [X] T018 [P] [US1] Verify `KnowledgeLeakageIT` (SC-004, both layers).
...
- [X] T019 [US1] Verify `KnowledgeRetrievalService` (the Open Host Service, R10).
...
- [X] T020 [US1] Verify `EmbeddingIndexer` and SPI wiring.
...
- [X] T021 [US1] Verify the envelope knowledge slot.
...
- [X] T022 [US1] Verify same-transaction snapshot write.
...
- [X] T023 [US1] Verify injected-knowledge tokens count against the case budget.
...
- [X] T024 [US1] Verify the replay harness reconstructs knowledge from snapshots.
  - ↳ Module: `replay/`.
...
- [X] T025 [US1] Verify the KnowledgeItem seed loader.
...
- [X] T026 [US1] Verify the operator read surface for items.
...
- [X] T027 [P] [US2] Verify `CapturePromotionIT` (SC-005/006).
...
- [X] T028 [P] [US2] Verify the `CaptureCandidate` entity + state machine.
...
- [X] T029 [P] [US2] Verify `PrefilterFinding` and `PromotionGateRecord` entities + repositories.
...
- [X] T030 [US2] Verify `CaptureService` (harvest, FR-008/R4).
...
- [X] T031 [US2] Verify `SensitivityPreFilter` (deterministic, R7/T3).
...
- [X] T032 [US2] Verify `RedactionService` (new revision, FR-011/012).
...
- [X] T033 [US2] Verify `PromotionGateService` (default-deny order + publish/reject).
...
- [X] T034 [US2] Verify the capture BPMN process (R6, standalone).
...
- [X] T036 [P] [US2] Verify the BPMN service-task delegates.
...
- [X] T037 [US2] Verify the Curator governance assets are seeded (FR-021, Principle I).
...
- [X] T038 [US2] Verify the candidate governance API.
...
- [X] T039 [P] [US3] Verify `DeprecationIT` (SC-007).
...
- [X] T040 [US3] Verify `DeprecationService` (single-transaction, R8).
...
- [X] T041 [US3] Verify DEPRECATED items are excluded from new retrieval.
...
- [X] T042 [US3] Verify the deprecate endpoint.
...
- [X] T043 [US3] Verify the affected-execution review surface.
...
- [X] T044 [P] [US4] Verify `InfluenceKpiIT` (SC-008).
...
- [X] T045 [US4] Verify `InfluenceEvaluationService` (paired runs, R9).
...
- [X] T046 [US4] Verify the KPI emission + V15 schema widening.
...
- [X] T047 [US4] Verify the influence API.
...
- [X] T048 [P] Verify architecture rules (persona ↛ knowledge; gateway-only providers).
...
- [X] T049 [P] Re-run Phase 1/2 suites unchanged with knowledge active (SC-009).
...
- [X] T050 Run the quickstart.md success checklist as the phase exit gate.
