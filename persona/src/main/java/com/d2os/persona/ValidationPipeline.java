 ...
 * rubric scoring from the pinned RubricDefinition's criteria, and the injection-symptom gate
 * (T1-b) — any one of which can produce a critical failure. Passes only when the weighted score is
 * ≥80% AND no criterion (including the injection gate) is a critical failure.
 *
 ...
 * second Gateway call per attempt (cost/latency) and its own reproducibility snapshot (Principle II)
 * that nothing here yet needs. Instead, {@code structural_completeness} and {@code content_quality}
 * — the two criteria every seeded Phase 1/2 rubric uses — are scored from real, deterministic
 * multi-signal analysis (sentence structure, degenerate-content detection, vocabulary diversity),
 * not merely character count: a heuristic based on length alone would (and previously did) pass
 * garbage like 20 repeated characters just for being "long enough." Deterministic scoring also keeps
 * the dozens of existing integration tests — which all depend on {@code StubAiGatewayClient}'s fixed
 * output passing validation — reproducible; an AI-judge would risk flaking them. Any other rubric
 * criterion name (e.g. Phase 3's {@code sensitivity_excluded}) keeps its original non-empty-output
 * fallback, unchanged by this task's scope.
 */
@Component
public class ValidationPipeline {
     ...
     * sentence AND not degenerate filler (e.g. one character repeated to hit a length floor — the
     * old pure-length heuristic would have wrongly passed that).
     */
    ...
    private double scoreContentQuality(String output) {
        ...
        return LENGTH_WEIGHT * lengthFactor + SENTENCE_WEIGHT * sentenceFactor + DIVERSITY_WEIGHT * diversityFactor;
    }
    ...
    private boolean isDegenerate(String text) {
        ...
        return (distinctChars.size() / (double) nonWhitespace.length()) < DEGENERATE_CHAR_DIVERSITY_FLOOR;
    }
