...
/**
 * One injected KnowledgeItem's provenance for a single OperationExecution (T014, V14, FR-006,
 * research R5/R9). Written by {@link OperationExecutionRecorder} in the SAME transaction as its
 * operation row, so an execution never exists without its full knowledge provenance. Immutable /
 * append-only.
 *
 * <p>{@code knowledgeItemId/key/version} are a SOFT reference (no FK): the snapshot must stay valid
 * and byte-identical even after the item is deprecated or superseded, and {@code knowledge_item} is
 * partitioned in another module (V14). {@code position} orders the item within the envelope's
 * knowledge slot so replay (T016) reconstructs the exact injected context.
 */
@Entity
...
public class KnowledgeInjectionSnapshot {
  ...
  public KnowledgeInjectionSnapshot(
      UUID id,
      UUID workspaceId,
  ...
  public UUID getOperationExecutionId() {
  ...
  public String getKnowledgeItemKey() {
    return knowledgeItemKey;
  }

  public int getKnowledgeItemVersion() {
