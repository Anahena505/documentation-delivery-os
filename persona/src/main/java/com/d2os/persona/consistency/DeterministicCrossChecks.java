...
/**
 * Tier-1 consistency checks (US3, FR-006/007/019): pure, deterministic, no AI. Parses each
 * validated persona output for a machine-readable index block and flags contradictions across the
 * WHOLE output set produced so far — the parallel specialists and the upstream sequential personas
 * (FR-019).
 *
 * <p>Index-block grammar (rendered by the persona templates, T014): lines of the form {@code
 * defines: ns:id, ns:id}, {@code references: ns:id, ns:id}, and {@code attr: name=value}. Two
 * findings are produced:
 *
 * <ul>
 *   <li>{@code DANGLING_REFERENCE} — an id referenced by one output that no output defines;
...
public class DeterministicCrossChecks {
  ...
  public DeterministicCrossChecks(
  ...
  public List<ConsistencyFinding> check(UUID caseId, UUID workspaceId) {
    ...
    for (PersonaInvocation inv :
        invocationRepository.findByCaseInstanceIdOrderBySequenceNoAsc(caseId)) {
            ...
            if (!ref.isBlank() && !definedIds.contains(ref)) {
              findings.add(
                  new ConsistencyFinding(
                      ...
                      workspaceId,
                      caseId,
                      ConsistencyFinding.Tier.DETERMINISTIC,
                      ConsistencyFinding.Kind.DANGLING_REFERENCE,
                      ref,
                      opId,
                      null,
                      "{\"reference\":\"" + esc(ref) + "\"}"));
          ...
          if (byValue.size() >= 2) {
            ...
            findings.add(
                new ConsistencyFinding(
                    ...
                    caseId,
                    ConsistencyFinding.Tier.DETERMINISTIC,
                    ConsistencyFinding.Kind.ATTRIBUTE_CONTRADICTION,
                    name,
                    first.getValue(),
                    second.getValue(),
                    "{\"attribute\":\""
