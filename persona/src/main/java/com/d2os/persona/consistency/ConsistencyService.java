 ...
 *       finding is DETERMINISTIC and blocking (FR-007). Each contradiction (two outputs) also
 *       writes a {@code trace_link} CONFLICTS_WITH edge (AD-7).
 *   <li><b>Semantic</b> — a real AI review run through the standard persona machinery ({@code
 *       consistency-reviewer}), so it is snapshot-recorded and replayable (Principle II). If its
 *       rubric fails, it escalates the Case (its own path) and a SEMANTIC finding is recorded as
 *       advisory (FR-008), never a hard block.
 * </ul>
 */
@Service
public class ConsistencyService {
  ...
  public ConsistencyService(
      DeterministicCrossChecks deterministicCrossChecks,
  ...
  /**
   * Runs both tiers. Returns true if the case is clean (no OPEN deterministic finding, semantic
   * passed).
   */
  @Transactional
    ...
    if (!semanticOk && reviewOp != null) {
      findingRepository.save(
          new ConsistencyFinding(
              ...
              workspaceId,
              caseId,
              ConsistencyFinding.Tier.SEMANTIC,
              ConsistencyFinding.Kind.SEMANTIC_INCOHERENCE,
  ...
  public boolean deterministicBlockingRemains(UUID caseId) {
    return findingRepository.countByCaseIdAndTierAndStatus(
            caseId,
            ConsistencyFinding.Tier.DETERMINISTIC.name(),
            ConsistencyFinding.Status.OPEN.name())
        > 0;
  }
  ...
  /**
   * Human decision on a finding (Principle V). A DETERMINISTIC finding cannot be WAIVED — it must
   * be fixed (RESOLVED after regeneration); WAIVE is rejected so a real contradiction can never be
   * waved through. Returns false if the waive rule is violated.
   */
  @Transactional
  public boolean resolveFinding(
      UUID caseId, UUID findingId, ConsistencyFinding.Status resolution, String actor) {
