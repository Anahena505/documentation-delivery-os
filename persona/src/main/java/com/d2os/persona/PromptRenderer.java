 ...
 * REFERENCE KNOWLEDGE block is prepended ahead of the rendered template. The delimiters make
 * explicit that the block is data, not instructions, so injected governed knowledge cannot smuggle
 * directives. With no injected knowledge the output is byte-identical to the pre-Phase-3 render
 * (backward compatible).
 */
@Component
public class PromptRenderer {
   ...
   * only inside these delimiters — so an uploaded file can never smuggle instructions into a
   * persona.
   */
   ...
   * is for a comment-and-regenerate re-entry — placed inside its own untrusted-data delimiters,
   * same framing as {@link #appendAttachmentSummaries}, so the comment text can never be read as an
