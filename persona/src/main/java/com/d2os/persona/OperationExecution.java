...
public class OperationExecution {
  ...
  // delivery/normal listings. Delivery packages are assembled from artifacts (never
  // operation_execution),
  // so an evaluation run cannot reach a package regardless; this flag marks it for operation-level
  // filters.
  @Column(nullable = false)
  ...
  /**
   * Mark this row as an influence-evaluation run (US4). Set before the single INSERT; never updated
   * after.
   */
  void markEvaluation() {
  ...
  public UUID getPersonaInvocationId() {
    return personaInvocationId;
  }
  ...
  public String getInjectedKnowledge() {
    return injectedKnowledge;
  }

  public String getPromptDefinitionVersion() {
    return promptDefinitionVersion;
  }
