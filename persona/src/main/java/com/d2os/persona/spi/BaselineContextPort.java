 ...
 * metadata-only reference is still genuine grounded context a persona can use to know WHAT baseline it
 * is being asked to delta/impact-analyze against, without the envelope literally re-authoring/copying
 * the baseline's content (R4's "reference, never copy" principle, applied conservatively here too).
 * Empty for a Case with no resolved baseline (every non-Enhancement case; or no
 * {@link BaselineContextPort} bean on the path, e.g. persona-only slice tests).
 */
