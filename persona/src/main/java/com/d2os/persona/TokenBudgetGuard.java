 ...
 * (T036, FR-017, T5-b). Both checks happen <em>before</em> the AI call — a Case is suspended before
 * it breaches either ceiling, never after, so no call is ever made once a ceiling would be crossed.
...
public class TokenBudgetGuard {
  ...
  /**
   * @throws TokenBudgetExceededException if the estimated call would exceed the Case or workspace
   *     budget; the Case is suspended as a side effect.
   */
  @Transactional
  public void checkBeforeCall(UUID caseId, long estimatedTokens) {
    ...
    if (kase.wouldExceedBudget(estimatedTokens)) {
      caseService.suspend(
          caseId, "token budget would be exceeded by estimated " + estimatedTokens + " tokens");
      throw new TokenBudgetExceededException(
          "case " + caseId + " suspended: token budget exceeded");
    }
    if (workspaceBudgetService.wouldExceed(kase.getWorkspaceId(), estimatedTokens)) {
      caseService.suspend(
          caseId,
          "workspace token budget would be exceeded by estimated " + estimatedTokens + " tokens");
      throw new TokenBudgetExceededException(
          "case " + caseId + " suspended: workspace token budget exceeded");
    }
  }
