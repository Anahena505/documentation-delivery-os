...
public class PersonaOutputAdapter implements PersonaOutputPort {
  ...
  public List<ValidatedOutput> validatedOutputsForCase(UUID caseId) {
    ...
    for (PersonaInvocation invocation :
        personaInvocationRepository.findByCaseInstanceIdOrderBySequenceNoAsc(caseId)) {
      if (!PersonaInvocation.Status.validated.name().equals(invocation.getStatus())) continue;

      List<OperationExecution> attempts =
          operationExecutionRepository.findByPersonaInvocationIdOrderByAttemptNoAsc(
              invocation.getId());
      ...
      outputs.add(
          new ValidatedOutput(
              personaKey,
              finalAttempt.getId(),
              finalAttempt.getOutputRef(),
              finalAttempt.getOutputHash()));
    }
    return outputs;
  }
}
