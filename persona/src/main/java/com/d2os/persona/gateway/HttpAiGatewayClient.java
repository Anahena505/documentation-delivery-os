 ...
 * mapping here — callers (personas) never change. Logs every call (T5-a) with metadata only (never
 * the full rendered prompt, to avoid leaking submission content into general logs — the full prompt
 * is preserved instead in the OperationExecution snapshot, T034).
 */
@Component
...
public class HttpAiGatewayClient implements AiGatewayClient {
  ...
  public HttpAiGatewayClient(AiGatewayProperties properties, WorkspaceRateLimiter rateLimiter) {
    ...
    this.concurrencyLimit =
        properties.maxConcurrentCalls() > 0
  ...
  private AiCallResult doCall(AiCallRequest request) {
    ...
    Map<?, ?> response = restClient.post().body(body).retrieve().body(Map.class);
    ...
    log.info(
        "AI Gateway call: provider={} model={} tokens={} elapsedMs={} promptChars={}",
        properties.provider(),
        properties.model(),
        tokensUsed,
        elapsedMs,
  ...
  public EmbedResult embed(EmbedRequest request) {
    ...
    try {
      ...
      Map<String, Object> body =
          Map.of(
              "model",
              properties.embedModel(),
              "input",
              List.of(request.text() == null ? "" : request.text()),
              "dimensions",
              expectedDims,
              "output_dimension",
              expectedDims);
      Map<?, ?> response = embedRestClient.post().body(body).retrieve().body(Map.class);
      ...
      // a cryptic Postgres dimension error on insert/query (T013). Never store/query a mismatched
      // vector.
      ...
      long elapsedMs = (System.nanoTime() - start) / 1_000_000;
      log.info(
          "AI Gateway embed: provider={} model={} dims={} elapsedMs={} inputChars={}",
          properties.provider(),
          properties.embedModel(),
          vector.length,
          elapsedMs,
