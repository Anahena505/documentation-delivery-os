...
/**
 * Per-workspace request-rate smoothing in front of provider calls (T037, T5-b). A sliding
 * one-minute window per workspace: when a workspace exceeds its requests-per-minute limit, the
 * caller is paced (a short block) rather than failed — the engine job pool still holds the Case, so
 * nothing is dropped, just slowed. In-memory is appropriate for the single deployable (PD-1); the
 * interface isolates it so a distributed limiter could replace it if the monolith is ever split.
 *
 * <p>Only the real {@link HttpAiGatewayClient} consults this — integration tests use the stub
 * gateway, so provider pacing never interferes with test timing.
 */
@Component
public class WorkspaceRateLimiter {
  ...
  public WorkspaceRateLimiter(
      @Value("${d2os.ai-gateway.default-rate-per-minute:600}") int defaultPerMinute) {
  ...
  public void acquire() {
    ...
    ConcurrentLinkedDeque<Long> window =
        windows.computeIfAbsent(workspaceId, k -> new ConcurrentLinkedDeque<>());
