...
/**
 * 008 T010 — fast unit test of {@link TokenBudgetGuard#checkBeforeCall} boundary behavior. No Spring
 * context, no DB: the repository/services are Mockito mocks, but the actual admit/reject decision is
 * driven by the REAL {@link CaseInstance#wouldExceedBudget(long)} logic
 * ({@code tokensSpent + estimatedTokens > tokenBudget}), constructed with a known budget.
 *
...
class TokenBudgetGuardTest {
    ...
    private CaseInstance caseWithBudget() {
        ...
        return new CaseInstance(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                "assessment", "1.0.0", BUDGET, "user:tester");
    }
    ...
    void usageOverCap_isRejectedAndCaseSuspended() {
        ...
        assertThrows(TokenBudgetExceededException.class,
                () -> guard.checkBeforeCall(caseId, BUDGET + 1));
