...
/**
 * 008 T011 — fast unit test of {@link WorkspaceScopeGuard#assertSameWorkspace}. No Spring context,
 * no DB: the {@link AuditWriter} {@link ObjectProvider} is a tiny hand-rolled stub whose {@code
 * getIfAvailable()} returns {@code null}, exercising the guard's "slice test without casecore" path
 * (the ERROR log is the audit signal). This isolates the real security decision — the
...
class WorkspaceScopeGuardTest {
  ...
  private static InjectedItem itemInWorkspace(UUID workspaceId) {
    return new InjectedItem(
        workspaceId, UUID.randomUUID(), "policy.key", 3, "some content", "sha256:deadbeef");
  }
  ...
  void foreignWorkspaceItem_isRejected() {
    ...
    KnowledgeScopeViolationException ex =
        assertThrows(
  ...
  void mixedItems_rejectOnFirstForeignItem() {
    ...
    assertThrows(
        KnowledgeScopeViolationException.class,
        () ->
            guard.assertSameWorkspace(
                caller, List.of(itemInWorkspace(caller), itemInWorkspace(foreign))));
  }
