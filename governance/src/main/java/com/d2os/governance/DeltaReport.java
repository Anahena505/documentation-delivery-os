...
public class DeltaReport {
    ...
    /** V26, T014: the prior published {@code DefinitionAsset} being superseded (definition-pair shape). */
     ...
     * toDefinitionId} must both be non-null (the CHECK constraint requires it); a first-ever
     * publish with no prior version to diff against has no legal row to write under this shape —
     * {@code DeltaReportService#generateForDefinitions}'s caller must skip generation entirely in
     * that case (documented in {@code studio.PublishService}). Distinguishable from the
     * artifact-triple constructor above by arity alone (6 args vs. 7), so no disambiguating flag
     * is needed.
     */
    public DeltaReport(UUID id, UUID workspaceId, UUID fromDefinitionId, UUID toDefinitionId,
                       String diffContent, String diffHash) {
        this.id = id;
        this.workspaceId = workspaceId;
    ...
    public UUID getFromDefinitionId() { return fromDefinitionId; }
    public UUID getToDefinitionId() { return toDefinitionId; }
