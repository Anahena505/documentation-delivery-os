 ...
 *
 * <p>State machine transitions are validated by {@link GateStatus#canTransitionTo}; every
 * transition writes a Decision + AuditEntry in the same transaction as the state change (Principle
 * V) — that write happens in {@code GateService}/{@code ReopenService} (later phases), not here.
 * This entity only guards the transition itself.
 */
@Entity
...
public class GateInstance {
   ...
   * ARTIFACT_REVISION} is the original Phase 5 subject kind (an {@code ArtifactRevision}); {@code
   * DEFINITION_VERSION} is Phase 6's addition — a {@code DefinitionAsset} version under studio
   * publish review. Matches this package's existing enum style ({@link GateStatus}).
   */
  public enum GateSubjectType {
    ARTIFACT_REVISION,
    DEFINITION_VERSION
  }
   ...
   *     workflow instance — so this column is nullable for those rows. Every {@code
   *     ARTIFACT_REVISION}-subject gate (the only kind before Phase 6) still always sets a real
   *     case id. Same column-only, FK-preserving relaxation casecore's V19 ({@code
  ...
  /**
   * @deprecated V26 read alias, kept for backward compatibility with existing callers (e.g. {@code
   *     GateTaskBridge}) and rows written before the polymorphic subject existed. New code should
   *     read {@link #subjectType}/{@link #subjectId} instead; this field is only still WRITTEN for
   *     {@code ARTIFACT_REVISION}-subject gates so it stays a faithful alias, never a second source
   *     of truth.
   */
  @Deprecated
  ...
  /**
   * {@code subject_type} (V26) — see {@link GateSubjectType}. Defaults to ARTIFACT_REVISION at the
   * DB level (matching every gate opened before this column existed).
   */
  ...
  public GateInstance(
    ...
    this(
        ...
        GateSubjectType.ARTIFACT_REVISION,
  ...
  /**
   * Polymorphic-subject constructor (V26, research R3, T4-b). {@code subjectType}/{@code subjectId}
   * are always set; the deprecated {@code subjectArtifactRevisionId} alias is ALSO set when {@code
   * subjectType == ARTIFACT_REVISION} (kept a faithful alias, never a second source of truth for a
   * DEFINITION_VERSION subject).
   */
  public GateInstance(
      UUID id,
      UUID workspaceId,
    ...
    this.subjectArtifactRevisionId =
        subjectType == GateSubjectType.ARTIFACT_REVISION ? subjectId : null;
  ...
  public GateType getGateType() {
    return GateType.valueOf(gateType);
  }

  public String getGateDefinitionKey() {
    return gateDefinitionKey;
  }

  public int getGateDefinitionVersion() {
    return gateDefinitionVersion;
  }
   ...
   */
  @Deprecated
  public UUID getSubjectArtifactRevisionId() {
    return subjectArtifactRevisionId;
  }

  public GateSubjectType getSubjectType() {
    return GateSubjectType.valueOf(subjectType);
  }
  ...
  public Integer getEscalationPolicyVersion() {
  ...
  public UUID getDeltaReportId() {
    return deltaReportId;
  }
