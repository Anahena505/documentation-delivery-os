 ...
 * decision has committed — mirrors {@code com.d2os.knowledge.capture.CaptureWaitReleaser} / {@code
 * CaptureWaitReleaserImpl} exactly: the interface lives here in {@code governance} so this module
 * stays Flowable-free (plan.md Structure Decision — "governance stays engine-agnostic"; engine
 * coupling is confined to {@code orchestration}'s {@code GateTaskBridge}, later enforced by an
 * ArchUnit rule, Phase 9 T048). {@code orchestration} supplies the implementation ({@code
 * EngineGateReleasePortImpl}), wrapping Flowable's {@code TaskService.complete(...)}, wired purely
 * via Spring DI (governance has no compile-time dependency on orchestration or Flowable).
 */
