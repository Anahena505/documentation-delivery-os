 ...
 * {@code create} event — it persists the {@link GateInstance} row and emits {@code GATE_OPENED},
 * both in the transaction {@code GateTaskBridge}'s {@code TaskListener} already opened.
 * Centralizing the row-creation + event-emission here (rather than in {@code GateTaskBridge}
 * directly, as the Phase 2 placeholder did) is what lets {@code GateEventPublisher} (T016) wire
 * cleanly off both {@code open} and {@code decide}.
 *
 * <p>{@code decide} accepts EXACTLY the three verbs APPROVE / REJECT / REQUEST_CHANGES (Q4) —
 * REOPEN is a distinct, later-phase operation ({@code ReopenService}, US3) never routed through
 * here. Writes the {@link DecisionRecord} + AuditEntry + outbox event in the SAME transaction as
 * the {@link GateInstance} state transition (Principle V), then best-effort releases the engine's
 * parked userTask via the {@link EngineGateReleasePort} SPI — keeping {@code governance} itself
 * Flowable-free (plan.md Structure Decision): the port interface lives here, {@code orchestration}
 * supplies the implementation wrapping {@code TaskService.complete(...)}, mirroring the existing
 * {@code CaptureWaitReleaser}/{@code CaptureWaitReleaserImpl} SPI pattern (Phase 3, T019/T026).
 */
@Service
public class GateService {
  ...
  /**
   * Polymorphic-subject overload (V26, research R3, T4-b, tasks.md T006): opens a gate against any
   * {@code (subjectType, subjectId)} pair — e.g. {@code (DEFINITION_VERSION, definition_asset.id)}
   * for the studio's D4 publish gate — rather than only an ArtifactRevision. Deliberately a NEW
   * overload, not a change to the existing {@code open} signature above: {@code GateTaskBridge}
   * (orchestration, the only current caller) keeps calling the original ARTIFACT_REVISION-shaped
   * overload unchanged. {@code decide(...)} needs no corresponding change — it is opaque to subject
   * type (research R3).
   */
  @Transactional
  public GateInstance open(
      ...
      String gateDefinitionKey,
      int gateDefinitionVersion,
      GateInstance.GateSubjectType subjectType,
      UUID subjectId,
      String inputsRef,
      String escalationPolicyKey,
      Integer escalationPolicyVersion,
   ...
   * and the gate transitions, all in this one transaction, before best-effort releasing the
   * engine's parked userTask.
   */
  @Transactional
  public GateInstance decide(UUID gateId, Verb verb, String actorId, String comments) {
    ...
    engineGateReleasePort.ifAvailable(
        port -> port.completeGateTask(gate.getEngineTaskId(), verb.name()));

    return gate;
  }

  /**
   * Non-self-review (FR-018, mirroring {@code PromotionGateService}'s D4 non-self-satisfiable
   * check). Gates review AI-drafted artifacts: persona execution has no human "author" field
   * anywhere in the schema ({@code operation_execution}/{@code persona_invocation} are
   * system-authored rows), so there is no direct producing-actor id to compare against. The closest
   * honest analog to "the artifact's own author" available here is the Case's own submitter ({@code
  ...
  private void requireNotSelfReview(GateInstance gate, String actorId) {
    ...
    caseInstanceRepository
        .findById(gate.getCaseInstanceId())
        .ifPresent(
            kase -> {
              if (actorId.equals(kase.getCreatedBy())) {
                throw new SelfReviewNotAllowedException(
                    "actor "
                        + actorId
                        + " is the case's own submitter and may not decide its gate "
                        + gate.getId());
              }
            });
  }
