 ...
 * {@code assessment-v2}'s callActivity currently maps those variables in — {@code GateTaskBridge}'s own
 * javadoc notes the parallel gap for {@code subjectArtifactRevisionId}) falls back to the seeded {@code
 * escalation-policy.default} — the same fallback role {@code d2os.governance.sla.default-durations}
 * describes (application.yml, T003).
 */
@Service
public class EscalationPolicyResolver {
    ...
    /** One step in the role chain: which role is notified at {@code stepIndex}, after {@code duration}. */
    ...
    public EscalationPolicyResolver(DefinitionResolutionService definitionResolution, ObjectMapper objectMapper) {
    ...
    public Optional<Policy> resolve(String key, Integer version) {
        Optional<DefinitionView> view = definitionResolution.latestPublishedView("ESCALATION_POLICY", key);
        if (view.isEmpty()) {
    ...
    private Policy toPolicy(DefinitionView view) {
        ...
        try {
            ...
            for (JsonNode s : stepsNode) {
                steps.add(new Step(s.path("stepIndex").asInt(), s.path("role").asText(),
