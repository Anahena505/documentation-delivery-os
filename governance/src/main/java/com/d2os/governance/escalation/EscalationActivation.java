 ...
 * Phase 6 US4, T033, research R4, Q9). Append-only at the DB grant level (no UPDATE/DELETE for {@code
 * d2os_app}) — firing is a fact, never edited.
 */
@Entity
...
public class EscalationActivation {
    ...
    public EscalationActivation(UUID id, UUID workspaceId, UUID gateInstanceId, String policyKey,
                                int policyVersion, int stepIndex, String role, boolean assigneeResolved) {
        this.id = id;
    ...
    public boolean isAssigneeResolved() { return assigneeResolved; }
