 ...
 * anywhere in this codebase yet — {@code GateController}'s own javadoc notes the same gap — so {@code
 * assigneeResolved} is honestly {@code false} today; the column exists so a future role model can flip
...
public class TimerFiredHandler {
    ...
    /**
     * Fired by {@code TimerEscalationDelegate} (orchestration) when the gate userTask's non-interrupting
     * boundary timer elapses. {@code stepIndex} identifies which role-chain step this timer represents
     * (currently always {@code 0} — the BPMN wires exactly one boundary timer per gate userTask; a
     * multi-step escalation chain would need one boundary timer per step, a later hardening pass).
     * A no-op (returns {@code null}) if the case has no currently-OPEN gate — the decision may have
     * already been made in the window between the timer firing and this handler running.
     */
    @Transactional
    public UUID onTimerFired(UUID workspaceId, UUID caseInstanceId, int stepIndex) {
        Optional<GateInstance> openGate = gateInstanceRepository
                .findByCaseInstanceId(caseInstanceId).stream()
                .filter(g -> g.status() == GateStatus.OPEN)
                .findFirst();
        if (openGate.isEmpty()) {
            return null;   // decided (or otherwise moved on) before the timer handler ran — nothing to escalate
        }
        ...
        String role = policy == null ? null : policy.step(stepIndex).map(EscalationPolicyResolver.Step::role).orElse(null);
        boolean assigneeResolved = false;   // no role/user-assignment model exists yet — always honestly false
        ...
        EscalationActivation activation = new EscalationActivation(UUID.randomUUID(), workspaceId, gate.getId(),
                policyKey, policyVersion, stepIndex, resolvedRole, assigneeResolved);
        activationRepository.save(activation);

        notificationService.notifyRole(workspaceId, resolvedRole, "governance", "SLA_ESCALATION",
    ...
    /** {@code definition_asset.version} is a semver-ish string (e.g. "5.0.0"); the major segment stands in for an int policy version. */
