 ...
 * calls for. It does NOT additionally drive {@code REOPENED -> OPEN} with a live re-activated
 * engine userTask: no existing BPMN mechanism in this codebase supports resuming a completed {@code
 * review-gate}/{@code approval-gate} callActivity instance (Flowable tasks are terminal once
 * completed), and building one (e.g. a dedicated re-entry event subprocess) is a BPMN redesign
 * beyond this delivery's scope. {@link GateStatus}'s own state-machine doc documents {@code
 * REOPENED -> OPEN} as a distinct, later edge — this method stops at {@code REOPENED}, honestly,
 * rather than faking an engine signal that would do nothing.
 */
@Service
public class ReopenService {
   ...
   * #reopen} checks for. At most one per {@code (gate, upstreamRevision)} pair (DB unique
   * constraint; a duplicate call is rejected by the constraint violation surfacing as a 500 today —
   * acceptable since the worklist UI only ever offers this action once per candidate).
   */
  @Transactional
  public ImpactAssessment recordImpactAssessment(
      UUID gateId,
  ...
  public GateInstance reopen(UUID gateId, String actorId) {
    ...
    GateReopenCandidate candidate =
        candidateRepository.findByGateInstanceId(gateId).stream()
            .filter(c -> c.getDisposition() != GateReopenCandidate.Disposition.DISMISSED)
            .findFirst()
            .orElseThrow(
                () ->
                    new NoSuchElementException("gate " + gateId + " has no reopen candidate row"));
  ...
  /**
   * Diff the candidate's stored (superseded) upstream revision against the artifact's current
   * latest.
   */
  private void attachDeltaReport(GateInstance gate, GateReopenCandidate candidate) {
    ArtifactRevision fromRevision =
        artifactRevisionRepository.findById(candidate.getUpstreamArtifactRevisionId()).orElse(null);
    if (fromRevision == null) {
      return; // upstream revision row no longer resolvable — reopen still proceeds without a delta
    }
    ArtifactRevision toRevision =
        artifactRevisionRepository
            .findFirstByArtifactIdOrderByRevisionNoDesc(fromRevision.getArtifactId())
            .orElse(null);
