...
/**
 * Identifies which already-APPROVED gates a revised upstream artifact invalidates (Phase 5 US3, T025,
 * research R3, FR-006/008). Walks {@code trace_link} outward from the superseded revision via BFS,
 * classifying each dependent by depth: {@code depth=1} (direct) is reopenable and, if an APPROVED gate
 * exists on that exact dependent revision, flips it {@code APPROVED -> REOPEN_CANDIDATE} (a system
 * transition, never a human decision — {@link GateStatus}'s own state-machine doc). {@code depth>1}
 * (transitive) is always {@code MANUAL_REVIEW} — never auto-reopened (Q3/AD-5) — and carries no {@code
 * gate_instance_id}, since "the approved gate to reopen" only makes sense one hop away.
 */
@Service
public class ReopenCandidateService implements ArtifactRevisionListener {
    ...
    public ReopenCandidateService(JdbcTemplate jdbcTemplate, ReopenDmnPort reopenDmnPort,
     ...
     * reopenDirectDependents} DMN says triggers a reopen. Returns every {@link GateReopenCandidate} row
     * written (depth-1 and transitive alike).
     */
    @Transactional
    public List<GateReopenCandidate> identifyReopenCandidates(UUID workspaceId, UUID upstreamRevisionId) {
        ...
        while (!frontier.isEmpty() && depth <= MAX_DEPTH) {
            ...
            for (UUID node : frontier) {
                for (Map<String, Object> edge : directDependents(workspaceId, node)) {
                    ...
                    GateReopenCandidate.Disposition disposition = depth == 1
                            ? GateReopenCandidate.Disposition.PENDING
                            : GateReopenCandidate.Disposition.MANUAL_REVIEW;

                    GateReopenCandidate candidate = new GateReopenCandidate(UUID.randomUUID(), workspaceId,
                            upstreamRevisionId, dependentRevisionId,
                            approvedGate == null ? null : approvedGate.getId(), depth, disposition);
    ...
    private GateInstance approvedGateFor(UUID artifactRevisionId) {
        return gateInstanceRepository
                .findBySubjectTypeAndSubjectId(GateInstance.GateSubjectType.ARTIFACT_REVISION.name(), artifactRevisionId)
                .stream()
