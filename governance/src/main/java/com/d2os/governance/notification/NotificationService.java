...
public class NotificationService {
    ...
    @Transactional
    public UUID notifyRole(UUID workspaceId, String recipientRole, String sourceModule, String type,
                           Map<String, Object> subjectRef, String message) {
        UUID id = UUID.randomUUID();
        jdbcTemplate.update(
                "INSERT INTO in_app_notification (id, workspace_id, recipient_role, source_module, type, subject_ref, message) "
                        + "VALUES (?, ?, ?, ?, ?, ?::jsonb, ?)",
                id, workspaceId, recipientRole, sourceModule, type, toJson(subjectRef), message);
        return id;
    }

    /** {@code GET /notifications} (T034, contracts/api.yaml) — the caller's roles, optionally unread only. */
    public List<Map<String, Object>> forRoles(UUID workspaceId, List<String> roles, boolean unreadOnly) {
        if (roles.isEmpty()) {
            return List.of();
        }
        String placeholders = String.join(",", roles.stream().map(r -> "?").toList());
        String sql = "SELECT id, recipient_role, source_module, type, subject_ref, message, created_at, read_at "
                + "FROM in_app_notification WHERE workspace_id = ? AND recipient_role IN (" + placeholders + ")"
                + (unreadOnly ? " AND read_at IS NULL" : "") + " ORDER BY created_at DESC";
    ...
    public void markRead(UUID notificationId) {
        jdbcTemplate.update("UPDATE in_app_notification SET read_at = now() WHERE id = ? AND read_at IS NULL",
                notificationId);
    }
