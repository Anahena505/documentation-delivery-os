...
public class PackageAssemblyService {
  ...
  public ExecutionPackage assemble(UUID workspaceId, UUID caseId) {
    ...
    if (openDeterministic != null && openDeterministic > 0) {
      throw new IllegalStateException(
          "cannot assemble package: "
              + openDeterministic
              + " open deterministic consistency finding(s)");
    }
    ...
    for (Artifact artifact :
        artifacts.stream()
            .sorted((a, b) -> a.getArtifactType().compareTo(b.getArtifactType()))
            .toList()) {
    ...
    ExecutionPackage pkg =
        new ExecutionPackage(UUID.randomUUID(), workspaceId, caseId, manifestJson, manifestHash);
  ...
  private void assertConditionalArtifactsFulfilled(UUID caseId, List<Artifact> artifacts) {
    ...
    Set<String> producedKinds =
        artifacts.stream().map(Artifact::getArtifactType).collect(Collectors.toSet());
    ...
    if (!unfulfilled.isEmpty()) {
      throw new IllegalStateException(
          "cannot assemble package: conditionally required artifact kind(s) not yet produced: "
              + unfulfilled);
    }
  }
