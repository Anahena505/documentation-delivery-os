...
public class PackageController {
    ...
    public PackageController(ExecutionPackageRepository packageRepository,
    ...
    public ResponseEntity<PackageResponse> get(@PathVariable UUID caseId) {
        ExecutionPackage pkg = packageRepository.findByCaseInstanceId(caseId)
                .orElseThrow(() -> new PackageNotDeliveredException("no package delivered for case " + caseId));
        HandoverRecord handover = handoverRepository.findByExecutionPackageId(pkg.getId())
                .orElseThrow(() -> new IllegalStateException("package " + pkg.getId() + " missing its handover record"));

        return ResponseEntity.ok(PackageResponse.from(pkg, handover,
                parse(pkg.getManifest()), parse(handover.getContentsIndex()), parse(handover.getArtifactHashes())));
    }
