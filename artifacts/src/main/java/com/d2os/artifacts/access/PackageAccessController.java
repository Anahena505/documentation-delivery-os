 ...
 * "reading a delivered package requires a grant" (FR-015) — no package-content-read endpoint existed
 * anywhere in this codebase before this phase to retrofit the check onto.
 */
@RestController
...
public class PackageAccessController {
    ...
    public PackageAccessController(PackageAccessService packageAccessService, ExecutionPackageRepository packageRepository) {
        this.packageAccessService = packageAccessService;
        this.packageRepository = packageRepository;
    }

    @GetMapping
    public ResponseEntity<PackageView> read(@PathVariable UUID packageId,
                                            @RequestHeader(value = "X-Roles", defaultValue = "reviewer") String roles) {
        ExecutionPackage pkg = packageRepository.findById(packageId)
        ...
        return ResponseEntity.ok(new PackageView(pkg.getId(), pkg.getCaseInstanceId(), pkg.getManifestHash()));
    }
    ...
    @PostMapping("/grants")
    public ResponseEntity<PackageGrantView> grant(@PathVariable UUID packageId,
            @RequestHeader(value = "X-Actor", defaultValue = "reviewer") String actor,
            @RequestBody GrantRequest request) {
        ExecutionPackage pkg = packageRepository.findById(packageId)
                .orElseThrow(() -> new NoSuchElementException("package " + packageId));
        PackageAccessGrant grant = packageAccessService.grant(pkg.getWorkspaceId(), packageId, request.role(), actor);
    ...
    public record PackageGrantView(UUID id, UUID packageId, String role, String grantedBy,
                                   OffsetDateTime createdAt, OffsetDateTime revokedAt) {
        static PackageGrantView of(PackageAccessGrant g) {
            return new PackageGrantView(g.getId(), g.getPackageId(), g.getRole(), g.getGrantedBy(),
                    g.getCreatedAt(), g.getRevokedAt());
        }
    }
}
