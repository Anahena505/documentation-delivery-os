...
public class HandoverRecordService {
    ...
    public HandoverRecordService(HandoverRecordRepository repository,
    ...
    @Transactional
    public HandoverRecord create(UUID workspaceId, ExecutionPackage pkg, UUID submissionRef,
        ...
        List<Map<String, String>> artifactHashes = artifacts.stream()
                .flatMap(a -> revisionRepository.findByArtifactId(a.getId()).stream()
                        .reduce((first, second) -> second).stream() // latest revision only
                        .map(rev -> Map.of("artifact", a.getArtifactType(), "hash", rev.getContentHash())))
                .toList();

        HandoverRecord record = new HandoverRecord(
                UUID.randomUUID(), workspaceId, pkg.getId(),
                toJson(contentsIndex), submissionRef, definitionSnapshotRef,
