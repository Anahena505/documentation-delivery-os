 ...
 * process (T022, FR-010). Runs {@link SensitivityPreFilter} over the candidate captured by the
 * preceding capture step (its id rides the process as the {@code candidateId} variable), which
 * records the PII/ sensitivity findings, redacts detected spans out of the content by default
 * (T3-c), and transitions the candidate CAPTURED → PREFILTERED.
 *
 * <p>Binds {@link WorkspaceContext} + RLS from the process {@code workspaceId} variable before
 * running — capture runs on the Flowable async job-executor thread pool with no HTTP request to
 * have set it, exactly as {@link PersonaStepDelegate} does for the initiation pipeline.
 */
