 ...
 * workflow routes here instead of the generic halt end event, then loops back to the SAME
 * {@code review-gate-call} callActivity so the reviewer is re-presented with a fresh gate cycle
 * (research R2's "REQUEST_CHANGES → gate REGENERATING → re-enter persona path → new gate cycle" loop).
 *
 ...
 * Does its work in several smaller transactional calls (mirroring {@link PersonaExecutionService}'s own
 * posture) rather than one big transaction, since {@code executePersona} makes an external AI Gateway
 * call it would be wrong to hold a DB connection open across.
 *
 * <p><b>What this class does NOT do</b>: it never opens the new {@code GateInstance} itself and never
 * writes artifact content directly. Re-entering {@code review-gate-call} spawns a fresh engine userTask,
 * whose {@code create} listener ({@link GateTaskBridge}) is the ONE place a {@code GateInstance} row is
 * ever created (T010/T014) — this delegate only produces the new ArtifactRevision + delta report and
 * hands the delta report id forward as a process variable ({@code regenerationDeltaReportId}) for that
 * bridge to attach once the new gate cycle actually opens (T020's "set the resulting delta_report.id
 * onto the new GateInstance.deltaReportId field"). The only artifact-content write path anywhere is
 * {@link ArtifactService#createRevision} (T023's API-surface scan asserts this).
 */
...
public class RegenerationDelegate implements JavaDelegate {
    ...
    public RegenerationDelegate(GateInstanceRepository gateInstanceRepository,
    ...
    public void execute(DelegateExecution execution) {
        ...
        try {
            ...
            GateInstance regeneratingGate = gateInstanceRepository
                    .findFirstByCaseInstanceIdAndStatusOrderByDecidedAtDesc(caseId, GateStatus.REGENERATING.name())
                    .orElse(null);
            ...
            ArtifactRevision priorRevision = artifactRevisionRepository.findById(priorRevisionId).orElse(null);
            ...
            boolean validated = personaExecutionService.executePersona(
                    caseId, personaKey, null, regeneratingGate.getReviewerComments());
            if (!validated) {
                return;   // exhausted the revise loop and escalated (PersonaExecutionService) — no new revision
            }
            ...
            if (newRevisionOpt.isEmpty()) {
                return;   // refused by the read-only guard — shouldn't happen (the original materialized fine)
            }
            ...
            if (!newRevision.getId().equals(priorRevisionId)) {
                ...
                DeltaReport deltaReport = deltaReportService.generate(
                        workspaceId, artifact.getId(), priorRevision, newRevision);
    ...
    private PersonaOutputPort.ValidatedOutput latestOutputFor(UUID caseId, String personaKey) {
        List<PersonaOutputPort.ValidatedOutput> outputs = personaOutputPort.validatedOutputsForCase(caseId);
