 ...
 * specialist branch is parked at its {@code <persona>-wait} receiveTask while its siblings finish
 * and the parallel join waits — no sibling output is discarded. A human resolution:
 *
 * <ul>
 *   <li><b>RETRY</b> — re-run the persona (a fresh validated invocation supersedes the failed one),
 *       then release the parked branch to the join;
 *   <li><b>ACCEPT_LAST_OUTPUT</b> — accept the last (failed) output as-is (mark the invocation
 *       validated so it still materializes into the package), then release the branch.
 * </ul>
 *
 ...
 * continuation run correctly scoped. The trigger uses the per-execution id (not a broadcast
 * signal), so only this branch resumes — sibling cases/branches are untouched.
 */
@Service
public class EscalationBridge {
  ...
  public boolean resolveBranch(UUID caseId, UUID invocationId, Action action, String rationale) {
    ...
    // persona is resolved when it has any validated invocation, regardless of an earlier
    // escalation.
