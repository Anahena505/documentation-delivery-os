 ...
 * pool in its own {@code REQUIRES_NEW} transaction with the workspace RLS context bound, so they run
 * with genuine wall-clock overlap and never block one another. The step joins on all four (FR-004),
 * then exposes {@code validated = all four passed} so the following gateway routes a partial failure
 * to the escalation wait state (FR-005) — every specialist's output is already persisted, so no
 * sibling work is lost.
 */
...
public class ParallelSpecialistsDelegate implements JavaDelegate {
    ...
    public ParallelSpecialistsDelegate(PersonaExecutionService personaExecutionService,
    ...
    public void execute(DelegateExecution execution) {
        ...
        progressEmitter.emit(workspaceId, caseId, ProgressEvent.Kind.BRANCH_FORKED, "analysis-specialists", null);

        List<Future<Boolean>> futures = SPECIALISTS.stream()
                .map(key -> executor.submit((Callable<Boolean>) () -> runSpecialist(caseId, workspaceId, key)))
                .toList();
        ...
        progressEmitter.emit(workspaceId, caseId, ProgressEvent.Kind.BRANCH_JOINED, "analysis-specialists", null);
