...
/**
 * Starts the standalone {@code knowledge-capture} process when a Case reaches {@code Delivered} (T020,
 * FR-008, research R6). The Case's {@code Delivered} transition writes an {@code event_outbox} row in the
 * same transaction (via {@code AuditWriter}); this consumer reacts to that delivery.
 *
 ...
 * consumer cannot flip {@code published_at} to mark a row consumed, and the outbox is RLS-scoped so a
 * scheduler thread cannot read every workspace's rows at once. So — exactly as {@link ReconciliationJob}
 * does — this sweep discovers work from Flowable's engine tables (which carry no RLS), reading the
 * {@code workspaceId} process variable to bind the correct RLS context before touching any domain table.
 * The initiation-v2 process instance completes precisely when the Case is delivered, so its finished
 * instances are the delivery signal.
 *
 * <p><b>Idempotency (no double-start):</b> both processes use the Case id as their business key, so a
 * Case already has a {@code knowledge-capture} instance (running or historic) with that key iff capture
 * was already triggered. The sweep skips those; {@code CaptureService} is additionally idempotent per
 * case (it returns existing candidates rather than re-harvesting), so an at-least-once trigger is safe.
 */
@Component
public class CaseDeliveredKnowledgeTrigger {
    ...
    public void maybeStartCapture(String businessKey) {
        ...
        boolean already = runtimeService.createProcessInstanceQuery()
        ...
        HistoricProcessInstance initiation = historyService.createHistoricProcessInstanceQuery()
                .processInstanceBusinessKey(businessKey)
                .processDefinitionKey(INITIATION_PROCESS_KEY)
