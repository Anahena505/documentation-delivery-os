...
/**
 * Engine-side bridge for a gate userTask's non-interrupting boundary SLA timer (Phase 6 US4, T032/T033,
 * research R4). Bound as a serviceTask right after the boundary timer in {@code
 ...
 * {@code governance}'s escalation package). {@code cancelActivity="false"} on the boundary event means
 * this execution branch runs concurrently with the still-parked {@code gate-decision} userTask — {@link
 * TimerFiredHandler} never touches that task or the {@code GateInstance} status (Principle V).
 */
...
public class TimerEscalationDelegate implements JavaDelegate {
    ...
    public TimerEscalationDelegate(TimerFiredHandler timerFiredHandler, WorkspaceRlsBinder workspaceRlsBinder) {
