 ...
 * fed into this gate) and materializes it via {@link ArtifactService#createRevision} — idempotent by
 * content hash (T019/T020's {@code ArtifactService} change), so re-entering the SAME gate cycle (or the
 * later approve-path re-materialization at {@code AssemblePackageDelegate}) never creates a duplicate
 * revision of unchanged content.
 *
 * <p>When {@code RegenerationDelegate} (T019) has just re-run the persona under review with reviewer
 * comments injected and produced a genuinely new revision, it leaves a {@code regenerationDeltaReportId}
 * process variable behind (carried across the callActivity boundary by the caller's {@code flowable:in}
 * mapping) — this bridge attaches that report to the newly-opened {@link GateInstance} (T020's "set the
 * resulting delta_report.id onto the new GateInstance.deltaReportId field") and emits {@code
 * GATE_REGENERATION_TRIGGERED} (T022) in the SAME transaction as the gate's own open, since this is the
 * one place a comment-and-regenerate loop's new gate cycle is actually created.
 */
...
public class GateTaskBridge implements TaskListener {
    ...
    public GateTaskBridge(GateService gateService, GateInstanceRepository gateInstanceRepository,
                          GateEventPublisher gateEventPublisher, ArtifactService artifactService,
                          PersonaOutputPort personaOutputPort, WorkspaceRlsBinder workspaceRlsBinder) {
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        try {
            ...
            GateInstance gate = gateService.open(workspaceId, caseInstanceId, gateType, gateDefinitionKey,
                    gateDefinitionVersion, subjectArtifactRevisionId, inputsRef, escalationPolicyKey,
                    escalationPolicyVersion, delegateTask.getId());
     ...
     * recently validated persona output — the one that just fed into this gate — so the gate always has
     * a real subject to review/regenerate against, matching {@code gate_instance.subjectArtifactRevisionId}'s
     * intent even though neither embedding workflow (T015) populates it explicitly. Fails open (returns
     * null) when the case has no validated persona output yet (e.g. a services-only slice test with no
     * pipeline behind it) — the gate still opens, just without a resolvable subject.
     */
    private UUID resolveSubjectArtifactRevision(UUID workspaceId, UUID caseInstanceId) {
        List<PersonaOutputPort.ValidatedOutput> outputs = personaOutputPort.validatedOutputsForCase(caseInstanceId);
        if (outputs.isEmpty()) {
            return null;
        }
        ...
        return artifactService.createRevision(workspaceId, caseInstanceId, caseInstanceId, last, kind)
