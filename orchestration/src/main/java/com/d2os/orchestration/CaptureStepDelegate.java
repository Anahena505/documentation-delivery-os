...
/**
 * BPMN service-task delegate for the capture step of the {@code knowledge-capture} process (T019/T021,
 * FR-008). Harvests the delivered case's lessons-learned candidate(s) via {@link CaptureService} and
 * pins the (first) candidate id onto the process as the {@code candidateId} variable so the downstream
 * pre-filter / curator steps operate on it.
 *
    ...
    public void execute(DelegateExecution execution) {
        ...
        try {
            ...
            if (candidateIds.isEmpty()) {
                throw new NoSuchElementException("no capture candidate produced for case " + caseInstanceId);
            }
