...
/**
 * Runs the Consistency-Check after the parallel join (US3, T030). Delegates to
 * {@link ConsistencyService} for the two tiers, then routes:
 * <ul>
 *   <li>a DETERMINISTIC contradiction hard-blocks — the Case is escalated for a human and the branch
 *       routes to the consistency wait state (FR-007);</li>
 *   <li>a failing SEMANTIC review has already escalated the Case (advisory, FR-008);</li>
 *   <li>a clean check flows straight on to the remaining personas.</li>
 * </ul>
...
public class ConsistencyCheckDelegate implements JavaDelegate {
    ...
    public ConsistencyCheckDelegate(ConsistencyService consistencyService,
    ...
    public void execute(DelegateExecution execution) {
        ...
        try {
            ...
            if (!clean && consistencyService.deterministicBlockingRemains(caseId)) {
                ...
                caseService.escalate(caseId, "consistency: deterministic cross-artifact contradiction blocks delivery");
            }
