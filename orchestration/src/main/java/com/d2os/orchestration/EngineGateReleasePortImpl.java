 ...
 * runtimeService.trigger}), the gate subprocess (T012/T013) uses a real Flowable userTask, so release
 * here means {@code TaskService.complete(...)} with the decided verb as the {@code gateDecision}
 * process variable — the callActivity subprocess's exclusive gateway (T015's embedding workflows)
 * reads that variable back via the callActivity's {@code flowable:out} mapping to route on the verb.
 */
@Component
