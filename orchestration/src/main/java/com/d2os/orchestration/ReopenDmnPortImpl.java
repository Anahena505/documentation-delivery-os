 ...
 * dmn/reopen-direct-dependents.dmn} (T024) — same resilience convention as every other DMN
 * evaluator in this codebase (e.g. {@code CaseTypeClassificationService}): an unmatched row,
 * undeployed decision, or engine hiccup degrades to "does not trigger" rather than failing the
 * caller.
 */
@Component
public class ReopenDmnPortImpl implements ReopenDmnPort {
  ...
  public boolean triggersReopen(String edgeKind) {
    ...
    return rows != null
        && rows.stream().anyMatch(r -> Boolean.TRUE.equals(r.get("triggersReopen")));
  }
}
