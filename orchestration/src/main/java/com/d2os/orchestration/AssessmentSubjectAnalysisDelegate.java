 ...
 * used instead of a native BPMN parallel gateway — embedded-Flowable process-instance optimistic-lock
 * contention would otherwise serialize the branches) — the pattern is reused exactly, only the
 * persona-key roster differs, since {@code ParallelSpecialistsDelegate}'s roster is hardcoded to
 * Initiation's four specialists and isn't parameterized.
 *
 * <p>The exclusive gateway that follows this step and the {@code specialists-wait} receiveTask it
 * routes a partial failure to reuse the SAME activity id convention as {@code initiation-v2.bpmn20.xml}
 * (both processes name their wait state {@code specialists-wait}), so {@link EscalationBridge} — whose
 * {@code findWaitExecution} resolves by process-instance (caseId) + that activity id — needs no changes
 * to resolve escalations raised by an Assessment case's parallel block.
 */
...
public class AssessmentSubjectAnalysisDelegate implements JavaDelegate {
    ...
    public AssessmentSubjectAnalysisDelegate(PersonaExecutionService personaExecutionService,
                                             WorkspaceRlsBinder workspaceRlsBinder,
                                             ProgressEmitter progressEmitter,
                                             PlatformTransactionManager transactionManager,
                                             @org.springframework.beans.factory.annotation.Value(
                                                 "${d2os.orchestration.specialist-pool-size:16}") int poolSize) {
    ...
    public void execute(DelegateExecution execution) {
        ...
        progressEmitter.emit(workspaceId, caseId, ProgressEvent.Kind.BRANCH_FORKED, "subject-analysis-specialists", null);

        List<Future<Boolean>> futures = SUBJECT_ANALYSTS.stream()
                .map(key -> executor.submit((Callable<Boolean>) () -> runAnalyst(caseId, workspaceId, key)))
                .toList();
        ...
        progressEmitter.emit(workspaceId, caseId, ProgressEvent.Kind.BRANCH_JOINED, "subject-analysis-specialists", null);
    ...
    private boolean runAnalyst(UUID caseId, UUID workspaceId, String personaKey) {
        ...
        try {
            ...
            return Boolean.TRUE.equals(tx.execute(status -> {
                workspaceRlsBinder.bindCurrentTransaction(workspaceId);
                return personaExecutionService.executePersona(caseId, personaKey, "branch-" + personaKey);
            }));
        } finally {
