...
public class AssemblePackageDelegate implements JavaDelegate {
  ...
  public AssemblePackageDelegate(
  ...
  public void execute(DelegateExecution execution) {
    ...
    try {
      ...
      List<ArtifactRevision> revisions =
          artifactService.materializeForCase(kase.getWorkspaceId(), caseId);
      ...
      handoverRecordService.create(
          kase.getWorkspaceId(), pkg, kase.getSubmissionId(), snapshot.getId());

      // KPIs (FR-015): completeness = artifacts delivered; cost = tokens spent this Case.
      kpiEmitter.emit(
          kase.getWorkspaceId(), KpiEmitter.PACKAGE_COMPLETENESS, caseId, revisions.size());
      kpiEmitter.emit(
          kase.getWorkspaceId(), KpiEmitter.CASE_COST_TOKENS, caseId, kase.getTokensSpent());
