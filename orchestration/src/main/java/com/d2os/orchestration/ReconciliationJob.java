 ...
 * Case still reads Running. That case would otherwise hang forever; the sweep marks it Escalated so
 * a human is pulled in, and records an auditable {@link ReconciliationRun}. It reads Flowable's
 * engine tables (which carry no RLS) to discover the stuck job, then binds the case's own workspace
 * before touching any RLS-scoped domain table. Healthy cases (including branches legitimately
 * parked at an escalation {@code receiveTask}) have no dead-letter jobs, so a clean sweep writes
 * nothing.
 */
@Component
public class ReconciliationJob {
  ...
  @Scheduled(
      fixedDelayString = "${d2os.orchestration.reconciliation-interval-ms:60000}",
    ...
    jobMetrics.time(
        ...
        () -> {
          ...
          for (Job job : deadLetterJobs) {
            try {
              reconcileDeadLetter(
                  job.getId(), job.getProcessInstanceId(), job.getExceptionMessage());
            } catch (Exception e) {
              log.warn(
                  "reconciliation failed for process {}: {}",
  ...
  public void reconcileDeadLetter(String jobId, String processInstanceId, String exceptionMessage) {
    ...
    try {
      ...
      String engineState =
          "{\"deadLetterJob\":true,\"transient\":"
              + transientConflict
              + ",\"exception\":"
              + jsonString(exceptionMessage)
              + "}";
      ...
      if (transientConflict) {
        ...
        reconciliationRepository.save(
            new ReconciliationRun(
                ...
                workspaceId,
                caseId,
                ReconciliationRun.Divergence.DEAD_LETTER_JOB,
                engineState,
                domainState,
                ReconciliationRun.Action.REPAIRED));
        progressEmitter.emit(
            workspaceId,
            caseId,
            ProgressEvent.Kind.RECONCILED,
            null,
        ...
        reconciliationRepository.save(
            new ReconciliationRun(
                ...
                engineState,
                domainState,
                ReconciliationRun.Action.ESCALATED));
