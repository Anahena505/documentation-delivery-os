 ...
 * {@code CaseController}'s {@code GET /cases/{id}/baseline} (T025) reads from. Lives in orchestration
 * (not persona, which defines the port, nor casecore, which owns the audit table) because persona
 * already depends on casecore and orchestration depends on both — the same dependency-inversion shape
 * {@code IntakeAttachmentSummaryPort} uses for {@link com.d2os.persona.spi.AttachmentSummaryPort}.
 */
@Component
public class BaselineContextPortImpl implements BaselineContextPort {
    ...
    public BaselineContextPortImpl(AuditEntryRepository auditEntryRepository, ObjectMapper objectMapper) {
    ...
    private List<String> summarize(AuditEntryRecord entry) {
        ...
        try {
            ...
            for (JsonNode r : revisions) {
                summaries.add(r.path("artifactType").asText() + " (revision " + r.path("revisionNo").asInt()
                        + ", content-hash " + r.path("contentHash").asText() + ")");
            }
        } catch (Exception e) {
            return List.of();
        }
