...
/**
 * Resolves a branch-level persona escalation from the parallel block (T023, FR-005,
 * contracts/api.yaml).
 *
 * <p>Distinct from the Phase 1 {@code EscalationController} (case-level, {@code /escalations/...}):
 * this operates on a specific parallel-branch {@code invocationId} and releases that branch's
 * parked receiveTask so the join can proceed, retaining every sibling's output. Path uses {@code
 * /branches} to avoid ambiguous mapping with the case-level resolver.
 */
@RestController
public class BranchEscalationController {
  ...
  public ResponseEntity<Void> resolve(
    ...
    boolean released =
        escalationBridge.resolveBranch(caseId, invocationId, action, request.rationale());
