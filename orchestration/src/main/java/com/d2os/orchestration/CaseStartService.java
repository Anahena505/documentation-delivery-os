 ...
 * Flowable process, and records the engine↔Case correlation in {@code workflow_instance}. All in one
 * transaction, so a failed engine start rolls back the state change.
 *
 * <p>Phase 4 (T014/T019): which BPMN process to start is no longer a single hardcoded key — with
 * Assessment (and, later, Enhancement) shipping their own {@code workflow.<key>} bindings, the process
 * definition key MUST come from the Case's own pinned {@link CaseDefinitionSnapshot} (AD-4), same as
 * every other definition a running Case touches. {@link #resolveProcessDefinitionKey} reads the
 * snapshot's {@code workflow} entry and looks up its frozen body for {@code processDefinitionKey}.
 * Initiation is unaffected: its pinned workflow entry resolves to {@code initiation-v2}, the same value
 * this class used to hardcode.
 */
@Service
public class CaseStartService {

    // Legacy fallback only — every case type's dependsOn (Initiation included) has pinned a `workflow`
    // entry since Phase 1/2, so this should never actually be reached; kept so a snapshot that somehow
    ...
    private String resolveProcessDefinitionKey(UUID caseId) {
        ...
        try {
            ...
            for (JsonNode entry : entries) {
                ...
                DefinitionView workflowDef = definitionLookupService.byTypeKeyVersion("workflow", key, version)
                        .orElseThrow(() -> new IllegalStateException(
                                "case " + caseId + " pins workflow " + key + "@" + version + " but it is not resolvable"));
                String processDefinitionKey = objectMapper.readTree(workflowDef.body())
                        .path("processDefinitionKey").asText();
