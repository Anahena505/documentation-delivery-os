...
public class CompatibilityMatrixService {
    ...
    public record CompatibilityEntry(String definitionType, String definitionKey, String definitionVersion,
                                     String dependencyType, String dependencyKey, String declaredRange,
                                     String resolvedVersion, boolean inRange) {}

    private final DefinitionAssetRepository repository;
    private final DefinitionResolutionService definitionResolution;
    private final ObjectMapper objectMapper;

    public CompatibilityMatrixService(DefinitionAssetRepository repository,
    ...
    public List<CompatibilityEntry> evaluate() {
        ...
        for (DefinitionAsset def : repository.findByStatus(DefinitionAsset.Status.Published.name())) {
            ...
            for (JsonNode decl : declarations) {
                ...
                String resolvedVersion = definitionResolution.latestPublished(depType, depKey)
                        .map(DefinitionRef::version).orElse(null);
                boolean inRange = resolvedVersion != null && inRange(range, resolvedVersion);
                matrix.add(new CompatibilityEntry(def.getType(), def.getKey(), def.getVersion(),
                        depType, depKey, range, resolvedVersion, inRange));
            }
        }
        return matrix;
    }
