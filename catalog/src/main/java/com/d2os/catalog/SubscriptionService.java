...
public class SubscriptionService {
    ...
    public SubscriptionService(DefinitionAssetRepository repository, CatalogAuditWriter auditWriter,
    ...
    public SubscriptionResult subscribe(UUID sourceDefinitionId, UUID workspaceId, String actor) {
        DefinitionAsset source = repository.findById(sourceDefinitionId)
                .orElseThrow(() -> new NoSuchElementException("definition " + sourceDefinitionId));
        if (!SYSTEM_WORKSPACE.equals(source.getWorkspaceId())) {
            throw new IllegalStateException("only Global (system-seeded) definitions may be subscribed to: "
                    + sourceDefinitionId + " belongs to workspace " + source.getWorkspaceId());
        }
        ...
        DefinitionAsset copy = new DefinitionAsset(UUID.randomUUID(), workspaceId, source.getKey(),
                source.getVersion(), source.getType(), "en", source.getBody(), actor);
        copy.recordCopyProvenance(sourceDefinitionId);
        String checksum = ChecksumUtil.sha256Hex(source.getBody());
        copy.markPublished(checksum);   // the copy is immediately Published — resolution prefers it (research R6)
        ...
        jdbcTemplate.update(
                "INSERT INTO library_subscription (id, workspace_id, source_definition_id, copied_definition_id, subscribed_by) "
                        + "VALUES (?, ?, ?, ?, ?)",
                UUID.randomUUID(), workspaceId, sourceDefinitionId, copy.getId(), actor);

        auditWriter.record(workspaceId, "definition_asset", copy.getId(), "LIBRARY_SUBSCRIBED", actor,
                Map.of("sourceDefinitionId", sourceDefinitionId.toString(), "checksumVerified", checksumVerified,
                        "key", source.getKey(), "version", source.getVersion()));
        return new SubscriptionResult(copy, checksumVerified);
    }

    /** Global (system-seeded) rows browsable in the library, with the caller workspace's own subscription state. */
    public List<Map<String, Object>> browseGlobal(UUID callerWorkspaceId) {
        return jdbcTemplate.queryForList(
                ...
                callerWorkspaceId, SYSTEM_WORKSPACE);
    }
    ...
    private boolean alreadySubscribed(UUID workspaceId, UUID sourceDefinitionId) {
        Long count = jdbcTemplate.queryForObject(
                ...
                Long.class, workspaceId, sourceDefinitionId);
