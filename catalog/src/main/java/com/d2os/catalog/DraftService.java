 ...
 * InReview-freeze and content-hash pinning described by data-model.md fall naturally out of
 * {@link DefinitionAsset#updateBody}'s Draft-only guard once a later task ({@code
 * PublishController}, T013) actually calls {@link DefinitionAsset#markInReview()}.
 *
...
public class DraftService {
     ...
     * uq_definition_type_key_version} unique constraint (V3) to surface a duplicate as a
     * persistence conflict — {@code DraftController} (T008) is what turns that into the spec's
     * 409, not this method.
     */
    @Transactional
    public DefinitionAsset create(String type, String key, String version, String body,
                                  UUID workspaceId, String actor) {
        DefinitionAsset draft = new DefinitionAsset(
                UUID.randomUUID(), workspaceId, key, version, type, "en", body, actor);
    ...
    /**
     * Replace a draft's body. Guarded to {@code Draft} status by {@link
     * DefinitionAsset#updateBody} itself (throws {@link IllegalStateException} otherwise) — this
     * single guard is what refuses edits both for an ordinary Published row and for a row
     * currently {@code InReview} (the "InReview freeze" data-model.md describes), since {@code
     * InReview} is a distinct, non-Draft status.
     */
    @Transactional
