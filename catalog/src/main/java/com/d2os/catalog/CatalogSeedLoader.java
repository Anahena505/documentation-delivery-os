...
/**
 * Seeds the runtime catalog into the reserved system workspace on startup, idempotent by {@code
 * (type, key, version)}.
 *
 ...
 * {@code case_type:initiation v2.0.0} whose {@code dependsOn} pins the full suite plus {@code
 * workflow:initiation-v2}. Because {@code CaseService} resolves the <em>latest</em> published
 * case_type, every new Case now runs the full suite via the {@code initiation-v2} process; v1 stays
 * available only for replay. Persona prose here is intentionally concise but real (charter,
 * competency, operation binding) — full catalog-studio authoring is a later phase, not this
 * loader's job.
 *
 * <p><b>Phase 4 (v4.0.0, scaffold only — T003)</b> reserves the seed pass for the Assessment and
 * Enhancement case types (research R1). This pass is intentionally empty at T003: the actual {@code
 * case_type.assessment} / {@code case_type.enhancement} definitions and their
...
public class CatalogSeedLoader implements ApplicationRunner {
  ...
  // The canonical Initiation persona suite (§8 roster minus Knowledge Curator, deferred to Phase
  // 3).
  // `defines`/`references` seed the machine-readable index block each artifact renders (T014) so
  // the
  // Phase 2 deterministic consistency checker (US3) has namespaced ids to cross-check.
  private static final List<Persona> SUITE =
      List.of(
          new Persona(
              "intake-analyst",
              "Intake Analyst",
          ...
          new Persona(
              ...
              "Threat Model",
              "control:security",
              "component:architecture,endpoint:api"),
          new Persona(
              "ux-architect",
              "UX Architect",
          ...
          new Persona(
              ...
              "environment:infra",
              "component:architecture,entity:data"),
          new Persona(
              "qa-test-strategist",
              "QA/Test Strategist",
          ...
          new Persona(
              ...
              "Risk Register",
              "risk:governance",
              "requirement:brd,control:security"),
          new Persona(
              "delivery-planner",
              "Delivery Planner",
  ...
  public CatalogSeedLoader(
      DefinitionAssetRepository repository, DefinitionPublishService publishService) {
  ...
  private void seedPhase1() {
    ...
    Stream.of("persona-1", "persona-2", "persona-3")
        .forEach(
            personaKey -> {
              seed(
                  "persona",
   ...
   * {@code TemplateDefinition}s (7 revised from the v0 audit's NEEDS_REVISION/ADOPTED sources + 2
   * greenfield GAPs the audit flagged Phase-1-mandatory: Task Breakdown, Handover Record) and 3
  ...
  private void seedPhase2() {
    ...
    String dependsOnJson =
        deps.stream().map(d -> "\"" + d + "\"").reduce((a, b) -> a + "," + b).orElse("");
   ...
   * curation Playbook, a curation Rubric, and a redaction Prompt. These are catalog definitions
   * only (Principle I — provenance-disciplined, published, semver + checksum via {@link #seed});
   * the initial KnowledgeItem seed set is loaded separately by {@code knowledge}'s
   * KnowledgeSeedLoader (catalog cannot depend on knowledge — the dependency is one-way, knowledge
   * → catalog).
   */
  private void seedPhase3() {
    ...
    // Redaction prompt: same T1-a untrusted-data framing — the captured content is DATA, not
    // instructions.
    seed(
        "prompt",
  ...
  /**
   * The Phase 4 Assessment/Enhancement seed set (data-model.md "New Catalog Content"): {@code
   * case_type.assessment}, {@code case_type.enhancement}, their {@code workflow.assessment} /
   * {@code workflow.enhancement} bindings, {@code rule.case-type-classification}, {@code
   * rule.conditional-artifacts}, the new Template/Rubric/Prompt definitions, and any dependent
   * personas. US1's classification rule (T009), Assessment's full catalog content (US2, T015/T016 —
   * {@link #seedAssessment()}), and Enhancement's full catalog content (US3, T021/T022 — {@link
   * #seedEnhancement()}) are seeded here now; only the conditional-artifacts rule (US5) remains for
   * its own later phase (T030-T031).
   */
  ...
  /**
   * Binds {@code dmn/conditional-artifacts.dmn} (T030) as a published RuleDefinition, same {@code
   * decisionKey}/{@code engine} shape as {@code rule.case-type-classification} above, plus the
   * {@code template.dpia} TemplateDefinition its one seeded rule row currently references. Nothing
   * in any case type's {@code dependsOn} names {@code rule.conditional-artifacts} — it is evaluated
   * directly by key ({@code ConditionalArtifactService}, T032), not resolved through a case type's
   * snapshot dependency list, since it applies across case types rather than belonging to one.
   */
   ...
   * package's FINDINGS/RECOMMENDATION artifacts (research R2's allowlist; ArtifactService's {@code
   * deriveArtifactKind} maps the recommendation persona to RECOMMENDATION and everything else here
   * to FINDINGS).
   */
  private static final List<Persona> ASSESSMENT_SUITE =
      List.of(
          new Persona(
              "assessment-intake",
              "Assessment Intake",
          ...
          new Persona(
              ...
              "finding:capability,finding:gap,finding:risk"),
          new Persona(
  ...
  private void seedAssessment() {
    ...
    // T018 (Q2 guard exemption) — both read these flags back out of the pinned
    // CaseDefinitionSnapshot
    ...
    // producedBy == persona key) and stamps source_template_id + template_version onto the
    // revision.
    seed(
        "template",
        "assessment-findings",
   ...
   * this suite runs SEQUENTIALLY — plain {@code personaStepDelegate} service tasks, one after
   * another, exactly like Initiation's persona chain — rather than adding a second parallel-fan-out
   * delegate. Two delta-analysis specialists ({@code requirements-delta-analyst}, {@code
   * design-delta-analyst}) feed a consolidator persona whose key is literally {@code delta-doc}
   * (mirroring how {@code assessment-findings} is both a consolidator persona key AND {@code
   * template.assessment-findings}'s key), followed by the final {@code impact-analysis} persona
   * (mirroring {@code template.impact-analysis}).
   */
  private static final List<Persona> ENHANCEMENT_SUITE =
      List.of(
          new Persona(
              "requirements-delta-analyst",
              "Requirements Delta Analyst",
              ...
              "Requirements Delta Notes",
              "delta:requirements",
              "problem:enhancement-scope"),
          new Persona(
          ...
          new Persona(
              ...
              "Delta Document",
              "delta:consolidated",
              "delta:requirements,delta:design"),
          new Persona(
              "impact-analysis",
              "Impact Analyst",
  ...
  private void seedEnhancement() {
    ...
    // the read-only allowlist enforcement (T017/R2) is a no-op for it
    // (ArtifactService.createRevision
  ...
  private void seedPhase5() {
    ...
    seedInitiationV3();
    seedAssessmentV2();
    // enhancement-v2 is INTENTIONALLY NOT seeded here (T015 scope note): neither
    ...
    // workflow/case_type now would mean either superseding a v1 that was never published, or
    // fabricating
    // an Enhancement case type wholesale, neither of which is this task's job. Tracked as a
    // dependency
   ...
   * suite / dependsOn set as v2, PLUS the two gate SUBPROCESS DefinitionAssets. {@code
   * workflow:initiation-v2} / {@code case_type:initiation} v2.0.0 are left untouched (Principle I —
   * running v2 cases keep replaying against their pinned snapshot).
   */
   ...
   * assessment-v1.bpmn20.xml}. {@code workflow:assessment} / {@code case_type:assessment} v4.0.0
   * are left untouched (Principle I).
   */
   ...
   * skeleton alongside the existing {@code name}/{@code kind}/{@code producedBy} fields. The {@code
   * content} uses the {@code {{slot}}} convention {@code ArtifactService.renderContent} substitutes
   * deterministically at materialization time (studio {@code PromptEditorModel}'s slot model) — the
   * persona output body binds to {@code {{body}}}, with provenance slots for kind, persona key,
   * source hash, and template version. Additive: readers that only consume {@code kind}/{@code
   * producedBy} (CaseService.extractKind, required_artifact pinning) are unaffected.
   */
