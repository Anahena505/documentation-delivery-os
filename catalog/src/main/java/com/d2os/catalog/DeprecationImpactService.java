 ...
 *       unaffected by deprecation (Principle I) regardless of what this report says.
 *   <li><b>Definition-graph dependents</b> — other Published definitions whose {@code dependsOn}
 *       list names this {@code (type, key)}.
 *   <li><b>Subscription copies</b> — {@code library_subscription} rows whose {@code
 *       source_definition_id} is this definition (Phase 6 US4, T025's copy-on-subscribe).
 * </ol>
 */
@Service
public class DeprecationImpactService {
  ...
  public record SubscriptionCopy(
      UUID workspaceId, UUID copiedDefinitionId, java.time.OffsetDateTime subscribedAt) {}
  ...
  public record ImpactReport(
      ...
      String key,
      String version,
      List<PinnedCase> pinnedActiveCases,
      List<DependentDefinition> dependents,
  ...
  public DeprecationImpactService(
      DefinitionAssetRepository definitionAssetRepository, JdbcTemplate jdbcTemplate) {
  ...
  private List<SubscriptionCopy> subscriptionCopies(UUID definitionId) {
    ...
    return rows.stream()
        .map(
            r ->
                new SubscriptionCopy(
                    (UUID) r.get("workspace_id"),
                    (UUID) r.get("copied_definition_id"),
