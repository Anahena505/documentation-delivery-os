 ...
 *
 * <p>The threat: a hostile upload (zip-bomb, malformed PDF, parser-exploit payload) must never crash
 * the app, hang a request thread, or exhaust the heap — and its raw bytes must never reach a persona.
 * This extractor enforces three containment bounds around Tika's {@link AutoDetectParser}:
 * <ol>
 *   <li><b>Time</b>: the parse runs on a dedicated worker with a hard wall-clock timeout; on overrun
 *       the task is cancelled and the attachment is rejected, so the request thread is always released.</li>
 *   <li><b>Memory</b>: input is size-capped at upload; extracted output is capped at
 *       {@link #MAX_EXTRACTED_CHARS} via a write-limited handler, bounding heap use regardless of input.</li>
 *   <li><b>Failure</b>: ANY throwable — parse error, or a resource error a bomb might provoke — is
 *       caught and converted to {@link ExtractionException}; the caller marks the attachment
 *       {@code REJECTED} and moves on (default-deny, Principle V). Extraction failure never propagates.</li>
 * </ol>
 *
 * <p><b>Isolation note (honest scope):</b> extraction runs in-process, not in a separate address space.
 * The originally-planned Tika {@code ForkParser} (a child JVM) was not adopted because its API requires
 * a {@code Serializable} {@code ContentHandler} — which the standard body handlers are not — and its
 * classpath propagation is unreliable on JDK 9+, making it fail for every file in this runtime. The
 * bounds above give time/memory/crash containment in-process; true per-parse process isolation is a
 * documented production-hardening follow-up. Critically, the FR-015 guarantee that raw content never
 * reaches a persona does <em>not</em> depend on process isolation — it is enforced structurally by the
 * summary-only envelope slot ({@code AttachmentSummaryPort} / {@code ExecutionEnvelopeBuilder}).
 */
@Component
public class SandboxedExtractor {
    ...
    /**
     * Extract plain text from {@code bytes} within the containment bounds. Blocks up to the configured
     * {@code extraction-timeout}. Returns the extracted text (possibly empty for a valid but content-free
     * file, or truncated at the character cap). Throws {@link ExtractionException} on failure/timeout.
     */
