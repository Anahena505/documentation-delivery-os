 ...
 * policy, store the raw bytes in the object store, then run the sandboxed extract → summarize
 * pipeline so a persona only ever sees the sanitized summary. Boundary violations (size/type) are
 * rejected before any record exists; a post-acceptance extraction failure lands the attachment in
 * {@code REJECTED} with a reason and leaves the submission (and any Case) untouched.
 */
@Service
public class AttachmentService {
   ...
   * {@code status} reflects the outcome ({@code SUMMARIZED} on success, {@code REJECTED} if
   * extraction failed). Throws {@link AttachmentPolicyException} for boundary violations (no record
   * created).
   */
  @Transactional
  public Attachment accept(UUID submissionId, String filename, String contentType, byte[] bytes) {
    ...
    if (bytes.length > properties.getMaxSizeBytes()) {
      throw new AttachmentPolicyException(
          AttachmentPolicyException.Reason.OVERSIZE,
          "attachment %d bytes exceeds cap %d"
              .formatted(bytes.length, properties.getMaxSizeBytes()));
    }
    if (!properties.isAllowed(contentType)) {
      throw new AttachmentPolicyException(
          AttachmentPolicyException.Reason.DISALLOWED_TYPE,
