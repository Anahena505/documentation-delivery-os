 ...
 * NEVER consumes these bytes — only the sanitized {@link AttachmentSummary#getSummaryText()}
 * produced by the sandboxed extract → summarize pipeline. {@link #filename} is display-only and is
 * never used to derive a storage path (path-traversal defense — the object key is workspace-scoped
 * and random).
 */
@Entity
...
public class Attachment {
  ...
  public void markExtracting() {
    this.status = Status.EXTRACTING.name();
  }

  public void markSummarized() {
    this.status = Status.SUMMARIZED.name();
  }
