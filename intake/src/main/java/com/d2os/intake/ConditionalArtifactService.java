 ...
 * already registered by T002's classpath scan) — COLLECT hit policy, so every matching row
 * accumulates into the result rather than picking one winner (the classification DMN's
 * UNIQUE-and-single-result style doesn't apply here). Follows {@link
 * CaseTypeClassificationService}'s resilience convention: a DMN/engine hiccup (not deployed, no
 * matching rows, engine unavailable) degrades to an empty list — the case-type's BASE
 * required-artifact set is unaffected — rather than failing case creation.
 */
@Service
public class ConditionalArtifactService implements ConditionalArtifactPort {
  ...
  public ConditionalArtifactService(
      DmnDecisionService dmnDecisionService, ObjectMapper objectMapper) {
  ...
  public List<ConditionalArtifact> evaluate(String formDataJson) {
    ...
    for (Map<String, Object> row : rows) {
      ...
      result.add(
          new ConditionalArtifact(
              String.valueOf(templateKey),
              String.valueOf(artifactKind),
  ...
  /**
   * {@code formData.personalData}, same opaque-JSON/never-instructions handling as elsewhere
   * (AD-12).
   */
