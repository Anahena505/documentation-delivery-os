...
/**
 * Adapter implementing casecore's {@link SubmissionLookup} port from the intake read model. This is
 * the seam that lets casecore open a Case without depending on the intake module.
 *
 ...
 * lower-cased {@code confirmed_case_type} (the catalog's case_type definitions are keyed
 * lower-case, e.g. {@code "initiation"}, while the classification enum is upper-case, e.g. {@code
 * "INITIATION"}). {@link ProblemSubmission#confirm} keeps these columns in sync for callers still
 * using the legacy {@code /confirm-classification} endpoint, so both confirm paths are honored
 * uniformly here.
 */
@Component
    ...
    return repository
        ...
        .map(
            s -> {
              ...
              String caseTypeKey =
                  confirmed && s.getConfirmedCaseType() != null
                      ? s.getConfirmedCaseType().toLowerCase(Locale.ROOT)
                      : s.getClassificationCaseType();
              return new SubmissionInfo(
                  s.getId(), s.getWorkspaceId(), caseTypeKey, confirmed, s.getFormData());
            });
  }
}
