...
public class RetentionController {
  ...
  public RetentionPolicy get() {
    ...
    Map<String, Object> row =
        jdbcTemplate.queryForMap(
            "SELECT retention_years, retention_policy_notes FROM workspace WHERE id = ?",
            workspaceId);
    return new RetentionPolicy(
        (Integer) row.get("retention_years"), (String) row.get("retention_policy_notes"));
  }
  ...
  public ResponseEntity<RetentionPolicy> set(@RequestBody RetentionPolicy request) {
    ...
    UUID workspaceId = WorkspaceContext.require();
    jdbcTemplate.update(
        "UPDATE workspace SET retention_years = ?, retention_policy_notes = ? WHERE id = ?",
