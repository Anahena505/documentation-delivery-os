 ...
 * choke point for JWT signing/verification — {@link WorkspaceContextFilter} calls
 * {@link #validateAndExtractWorkspace(String)} on every request's {@code Authorization} header and
 * never touches signing material itself.
 *
 ...
 * by the configured issuer, and carries a syntactically valid UUID {@code workspace_id} claim.
 * Any other failure (bad signature, malformed token, missing claim) yields an empty result — callers
