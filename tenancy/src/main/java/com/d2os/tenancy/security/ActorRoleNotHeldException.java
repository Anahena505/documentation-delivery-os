 ...
 * actor_role} MUST be one the principal actually holds — a caller cannot stamp a decision with a role
...
public class ActorRoleNotHeldException extends RuntimeException {
  ...
  public ActorRoleNotHeldException(String userId, String requiredRole) {
    super("authenticated user '" + userId + "' does not hold the required role '" + requiredRole
        + "' for this decision");
  }
}
