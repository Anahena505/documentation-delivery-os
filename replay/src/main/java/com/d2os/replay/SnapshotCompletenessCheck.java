 ...
 *
 * <p>Phase 3 (T016, FR-007, research R5): additionally reconstructs the injected-knowledge slot from
 * the {@code knowledge_injection_snapshot} rows — the pinned {@code (key,version)} + content hash —
 * NOT the item's current state, so an item deprecated/superseded after the run cannot perturb replay.
 */
@Component
public class SnapshotCompletenessCheck {
    ...
    public SnapshotCompletenessCheck(KnowledgeInjectionSnapshotRepository injectionSnapshotRepository,
    ...
    /**
     * Reconstruct and verify the injected-knowledge slot from the snapshot (T016, FR-007): if the op's
     * {@code injected_knowledge} JSON is non-empty, snapshot rows must exist and their count must match;
     * positions must be contiguous from 0; and — the actual reproducibility guarantee — for every snapshot
     * the pinned {@code (key,version)} item must still be loadable and its content must re-hash to the
     * snapshot's {@code content_hash}. This catches the two ways byte-for-byte replay can silently break:
     * the pinned item version having been physically removed (nothing to reconstruct from), or its
     * {@code content} having drifted since the run (hash mismatch). A blank/absent hash also fails.
     * An operation that injected nothing trivially reproduces (empty slot, no stray rows).
     */
    public boolean knowledgeContextReproduced(OperationExecution op) {
        ...
        for (int i = 0; i < snapshots.size(); i++) {
            ...
            // Reconstruct from the PINNED version and re-verify the hash against the item's stored content.
    ...
    private String loadPinnedContent(KnowledgeInjectionSnapshot snap) {
        return jdbcTemplate.query(PINNED_CONTENT_SQL,
                rs -> rs.next() ? rs.getString(1) : null,
                snap.getWorkspaceId(), snap.getKnowledgeItemKey(), snap.getKnowledgeItemVersion());
    }
     ...
     * byte-identically the harness needs the model identity plus the SHA-256 of both the sandbox-extracted
     * input and the summary output. Any blank field is a reproducibility gap. Kept as a pure function so
     * the replay module needs no dependency on intake — the replay IT reads the persisted fields via JDBC
     * and passes them here.
     */
    public boolean attachmentSummarySnapshotComplete(String modelId, String modelVersion,
                                                     String extractedTextHash, String summaryHash) {
        return notBlank(modelId) && notBlank(modelVersion)
                && notBlank(extractedTextHash) && notBlank(summaryHash);
    }
