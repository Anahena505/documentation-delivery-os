 ...
 *
 * <p><b>Injected-knowledge reproduction</b> (T016, FR-007, research R5): for each operation the harness
 * loads its {@code knowledge_injection_snapshot} rows ordered by {@code position} and verifies the
 * injected context reconstructs from the SNAPSHOT (the pinned {@code (key,version)} + content hash),
 * never the item's current state — so an item deprecated or superseded after the run does not perturb
 * replay. A per-operation {@code knowledgeContextReproduced} flag surfaces this and feeds the report's
 * matched/complete accounting.
 *
 * <p><b>Parallel-case + consistency coverage</b> (Phase 2 US5, T049, SC-008, FR-016): the semantic
 * Consistency-Check reviewer runs as an ordinary persona step, so its OperationExecution is replayed by
 * exactly this path — a parallel case (four specialists + consistency reviewer) replays byte-identically
 * with no special handling. Attachment summaries live outside {@code operation_execution} (they are
 * produced pre-Case at upload time); their inline reproducibility snapshot is checked separately via
 * {@link SnapshotCompletenessCheck#attachmentSummarySnapshotComplete}.
 */
@Component
public class ReplayHarness {
    ...
    public ReplayReport replay(UUID caseId) {
        ...
        for (OperationExecution op : operations) {
            ...
            results.add(new ReplayReport.OperationResult(op.getId(), byteIdentical, complete, knowledgeReproduced));
        }
