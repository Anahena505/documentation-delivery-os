 ...
 * FR-003). Maps a Rubric definition's raw {@code body} JSON — {@code
 * {"personaKey":"...","criteria":[{"name":...,"weight":...,"critical":...}, ...]}} — into named,
 * typed fields the studio editor form binds to, instead of a free-form JSON blob.
 *
 * <p>The shape mirrors what {@code CatalogSeedLoader} actually seeds (e.g. {@code seed("rubric",
 * personaKey + "-rubric", V1, "{\"personaKey\":...,\"criteria\":[{\"name\":
 ...
 * runtime-compatible with the existing scoring pipeline the moment it is published (a later phase's
 * job, not this one's).
 */
public record RubricEditorModel(String personaKey, List<RubricCriterionField> criteria) {
  ...
  public static RubricEditorModel fromBodyJson(String bodyJson, ObjectMapper mapper)
      throws Exception {
    ...
    for (JsonNode c : root.path("criteria")) {
      criteria.add(
          new RubricCriterionField(
              c.path("name").asText(null),
              c.path("weight").asDouble(0.0),
              c.path("critical").asBoolean(false)));
    }
