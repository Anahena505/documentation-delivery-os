 ...
 * FR-002/FR-003). Maps a Prompt definition's raw {@code body} JSON — {@code
 * {"personaKey":"...","template":"..."}} — into a typed personaKey + template pair, plus the {@code
 * {{slotName}}} placeholders the template declares. Slots are DERIVED from the template text, not
 * separately stored, so the editor can render one named field per slot without a second source of
 * truth that could drift from the template.
 *
 * <p>The shape mirrors {@code CatalogSeedLoader}'s real seeded prompts (e.g. {@code seed("prompt",
 * personaKey + "-prompt", V1, "{\"personaKey\":...,\"template\":\"You are %s. ...
...
public record PromptEditorModel(String personaKey, String template) {
  ...
  /**
   * The distinct {@code {{slotName}}} placeholders declared in {@link #template}, in first-seen
   * order.
   */
  ...
  public static PromptEditorModel fromBodyJson(String bodyJson, ObjectMapper mapper)
      throws Exception {
    JsonNode root = mapper.readTree(bodyJson);
    return new PromptEditorModel(
        root.path("personaKey").asText(null), root.path("template").asText(null));
  }
}
