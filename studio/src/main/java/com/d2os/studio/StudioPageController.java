...
/**
 * Thymeleaf page-serving controller (tasks.md T011, research R1). A plain {@code @Controller}
 * (not {@code @RestController}) resolving server-rendered views over the same
 * {@link DraftService}/{@link DefinitionAssetRepository} the JSON API ({@link DraftController},
 * T008) uses.
 *
 * <p>This phase proves the presentation-layer plumbing works — Spring MVC route -&gt; model
 * -&gt; Thymeleaf template, referencing the T002 vendored asset paths — not a polished, fully
 * interactive UI. The dmn-js decision-table island and any live diff2html rendering are left
 * genuinely inert until a later, network-enabled step actually vendors those files (see
 * {@code static/studio/vendor/README.md}); the templates reference the paths they will load from
 * so nothing needs to change once the files land.
 */
@Controller
...
public class StudioPageController {
    ...
    public StudioPageController(DefinitionAssetRepository repository, DraftService draftService,
    ...
    public String list(@RequestParam(required = false) String type, Model model) {
        List<DefinitionAsset> drafts = repository.findAll().stream()
    ...
    /** {@code GET /studio/drafts/{draftId}} — the per-type editor page (DMN table for rules, plain form otherwise). */
    @GetMapping("/{draftId}")
    ...
    /**
     * {@code POST /studio/drafts/{draftId}} — the plain-HTML-form save action backing the editor
     * page above (browsers cannot submit HTML forms with method PUT; the JSON API's {@code PUT
