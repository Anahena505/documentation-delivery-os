 ...
 * T008/T018), same convention as {@code GateExceptionHandler}/{@code CaseExceptionHandler}/ {@code
 * PackageExceptionHandler} (each module's own {@code @RestControllerAdvice}, all coexisting in the
 * same {@code :app} context — Spring resolves one matching handler per exception without ambiguity
 * across beans). Extended (rather than given a sibling handler) for {@link
 * PublishConflictException} (T018) since it maps to the exact same 409/{@code ProblemDetail} shape
 * as {@link DraftConflictException} — one class, no duplicated boilerplate.
 */
...
public class DraftExceptionHandler {
   ...
   * pinned-content-hash tamper, or duplicate-tuple conflicts from {@link PublishService} — always a
   * 409, same as {@link DraftConflictException}.
   */
