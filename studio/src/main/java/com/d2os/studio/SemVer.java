 ...
 * detection for the second architecture-board gate (FR-007) — every version this codebase seeds or
 * accepts through the studio is a plain {@code X.Y.Z} (see {@code CatalogSeedLoader}, {@code
 * StudioAuthoringIT}); pre-release/build-metadata suffixes are rejected outright ({@link
 * IllegalArgumentException}, surfaced by {@code PublishService} as a 409 rather than silently
 * truncated or mis-ordered).
 */
