 ...
 *
 * <p>A dedicated page controller (plan.md/T015 offered either this or a {@code
 * StudioPageController} method) — kept separate because it reads from BOTH {@code catalog} ({@link
...
public class ReviewPageController {
  ...
  public String review(@PathVariable UUID gateId, Model model) {
    ...
    if (gate.getSubjectType() == GateInstance.GateSubjectType.DEFINITION_VERSION
        && gate.getSubjectId() != null) {
      definitionAssetRepository
          .findById(gate.getSubjectId())
          .ifPresent(d -> model.addAttribute("subject", d));
    }

    DeltaReport report =
        gate.getDeltaReportId() == null
            ? null
