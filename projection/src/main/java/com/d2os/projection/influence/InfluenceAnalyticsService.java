 ...
 *
 * <p>Runs synchronously inside an HTTP request ({@link com.d2os.projection.api.InfluenceController}),
 * same posture as {@link com.d2os.projection.query.TraceabilityQueryService}: no explicit RLS
 * binding here because {@code WorkspaceContextFilter} already bound the session for this request
 * (unlike the background jobs — {@code Projector}/{@code RebuildJob}/{@code CycleDetector} — which
 * must bind it themselves).
 */
@Service
public class InfluenceAnalyticsService {
    ...
    public enum State { MEASURED, NOT_YET_MEASURABLE }

    /**
     * {@code delta}/{@code recordedAt} match contracts/api.yaml's {@code InfluenceEntry.readings}
     * shape ({@code kpi_sample.value}/{@code at} respectively). The contract's third field,
     * {@code operationDefinitionKey}, is NOT recorded anywhere in {@code kpi_sample} (verified — V9/
     * V15 carry no such column, and {@code dimensions} only ever holds {@code key}/{@code version});
     ...
     */
    public record InfluenceEntry(String knowledgeKey, int knowledgeVersion, State state,
                                 List<Reading> readings, List<UUID> touchedOperations, List<UUID> touchedArtifacts) {}
    ...
    public List<InfluenceEntry> analyze(UUID workspaceId, String knowledgeKey, UUID caseId) {
        ...
        List<Map<String, Object>> itemNodes = knowledgeKey == null
                ...
                : jdbcTemplate.queryForList(
                        ...
                        workspaceId, generation, knowledgeKey + ":%");
        ...
        for (Map<String, Object> itemRow : itemNodes) {
            ...
            List<UUID> artifactBusinessIds = producedArtifactIds(workspaceId, generation, operationBusinessIds);

            List<UUID> touchedOperations = resolveGraphNodeIds(workspaceId, generation,
                    "OPERATION_EXECUTION", operationBusinessIds);
            List<UUID> touchedArtifacts = resolveGraphNodeIds(workspaceId, generation,
                    "ARTIFACT_REVISION", artifactBusinessIds);
            ...
            entries.add(new InfluenceEntry(key, version, state, readings, touchedOperations, touchedArtifacts));
        }
        return entries;
    }
     ...
     */
    private List<UUID> resolveGraphNodeIds(UUID workspaceId, int generation, String nodeType, List<UUID> businessIds) {
    ...
    /** INJECTED_INTO edges: {@code itemNode -> OPERATION_EXECUTION node}, natural_key IS the operation execution id. */
    private List<UUID> injectedOperationIds(UUID workspaceId, int generation, UUID itemNodeId) {
        List<String> naturalKeys = jdbcTemplate.queryForList(
                "SELECT gn.natural_key FROM graph_edge ge JOIN graph_node gn ON gn.id = ge.to_node "
                        ...
                        + "AND ge.from_node = ?",
                String.class, workspaceId, generation, itemNodeId);
    ...
    private List<UUID> scopeToCase(UUID workspaceId, UUID caseId, List<UUID> operationIds) {
        ...
        return jdbcTemplate.queryForList(
                "SELECT oe.id FROM operation_execution oe "
                        ...
                        + "WHERE oe.workspace_id = ? AND pi.case_instance_id = ? AND oe.id IN (" + placeholders + ")",
                UUID.class, args);
    }

    /** PRODUCED edges FROM the given OPERATION_EXECUTION nodes -> ARTIFACT_REVISION nodes (business ids both sides). */
    private List<UUID> producedArtifactIds(UUID workspaceId, int generation, List<UUID> operationIds) {
        if (operationIds.isEmpty()) {
            return List.of();
        }
        String placeholders = String.join(",", operationIds.stream().map(id -> "?").toList());
        ...
        List<String> naturalKeys = jdbcTemplate.queryForList(
                "SELECT gn2.natural_key FROM graph_edge ge "
                        ...
                        + "AND gn1.node_type = 'OPERATION_EXECUTION' AND gn1.natural_key IN (" + placeholders + ")",
                String.class, args);
        return naturalKeys.stream().map(UUID::fromString).toList();
    }
    ...
    private List<Reading> readings(UUID workspaceId, String key, int version) {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                ...
                workspaceId, key, String.valueOf(version));
        List<Reading> readings = new ArrayList<>(rows.size());
        for (Map<String, Object> row : rows) {
            readings.add(new Reading(((Number) row.get("value")).doubleValue(), toOffsetDateTime(row.get("at"))));
        }
        return readings;
    }
