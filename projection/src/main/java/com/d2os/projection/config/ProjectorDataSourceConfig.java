 ...
 * bound to {@code spring.datasource.projector.*} (see application.yml) — the {@code
 * d2os_projector} role V28 provisions, the sole writer of the five graph tables (research R2,
 * Principle III). Deliberately NOT {@code @Primary}: the app's normal datasource (tenancy's {@code
 ...
 * already wires to the {@code @Primary} (app/{@code d2os_app}) datasource — that covers every
 * READ path (generation-filtered finder methods), and {@code d2os_app} has SELECT-only on the
 * graph tables per V28, which is exactly the right privilege for reads. Standing up a SECOND,
 ...
 * — is simpler, has no risk of the two EntityManagerFactories fighting over the same entity
 * classes, and is exactly as correct: every write still goes through the {@code d2os_projector}
 * connection, satisfying the sole-writer DB grant. This is a pragmatic choice, not JPA purity;
 * documented here per T007's own instruction to prefer correctness over ORM uniformity.
 *
 ...
 * {@code com.d2os.projection.ProjectorRlsBinder#bindCurrentTransaction}, the same {@code SET
 * LOCAL app.workspace_id} pattern {@code WorkspaceRlsBinder} uses, just issued against THIS
 * module's own {@link JdbcTemplate}/transaction manager instead of the app's. Actually invoking
 * that per-workspace loop is the {@code Projector}/{@code RebuildJob}'s job (T008/T009, a later
 * phase); T007 wires the datasource/binder so that work has something correct to call into.
 */
@Configuration
