...
/**
 * T018 &mdash; the self-contained traceability panel (research R7, FR-006). A plain {@code
 * @Controller} resolving a server-rendered Thymeleaf view over the exact same {@link
...
public class TraceabilityPanelController {
    ...
    /** {@code GET /projection/traceability} &mdash; renders the search form, and results once a query is submitted. */
    @GetMapping
    public String search(@RequestParam(required = false) String nodeType,
        ...
        boolean searched = nodeType != null && !nodeType.isBlank() && naturalKey != null && !naturalKey.isBlank();
        ...
        if (searched) {
            ...
            var result = "DEPENDS_ON".equals(relation)
                    ? traceabilityQueryService.dependsOn(workspaceId, nodeType, naturalKey, dir, maxDepth, pageToken)
                    : traceabilityQueryService.tracesTo(workspaceId, nodeType, naturalKey, dir, maxDepth, pageToken);
