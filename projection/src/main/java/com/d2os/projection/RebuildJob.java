 ...
 * FAIL (research R4/R5, FR-002/015). Scheduled per {@code d2os.projection.rebuild.schedule} (weekly
 * drift-detection default); also triggerable on demand via {@code POST /graph/admin/rebuild}
 * ({@link GraphAdminController}).
 *
 ...
 * Not a correctness bug for what gets SERVED, just wasted writes in a narrow window; real cross-job
 * coordination (e.g. an advisory lock) is left for later hardening if this proves to matter at
 * production scale.
 */
@Component
public class RebuildJob {
  ...
  private final ExecutorService onDemandExecutor =
      Executors.newFixedThreadPool(2, r -> new Thread(r, "graph-rebuild-on-demand"));
  ...
  /**
   * {@code POST /graph/admin/rebuild} — returns false (409) if a rebuild is already running for
   * this workspace.
   */
    ...
    onDemandExecutor.submit(
        () -> {
          try {
            ...
            log.error(
                "on-demand rebuild failed for workspace {}: {}", workspaceId, e.toString(), e);
          } finally {
        ...
        () -> {
          ...
          for (UUID workspaceId : workspaceIds) {
            ...
            try {
              ...
              log.error(
                  "scheduled rebuild failed for workspace {}: {}", workspaceId, e.toString(), e);
            } finally {
              inProgress.remove(workspaceId);
            }
          }
  ...
  /**
   * Synchronous build -> verify -> flip-or-drop for one workspace. Package-visible for direct test
   * invocation.
   */
  RebuildResult rebuild(UUID workspaceId) {
    ...
    if (currentGeneration == null) {
      ...
      log.info(
          "skipping rebuild for workspace {}: no live generation yet (not bootstrapped)",
          workspaceId);
    ...
    EquivalenceVerifier.EquivalenceResult equivalence =
        equivalenceVerifier.verify(workspaceId, newGeneration);
    ...
    if (equivalence.isPass()) {
      ...
      flipTx.executeWithoutResult(
          status -> {
            projectorRlsBinder.bindCurrentTransaction(workspaceId);
            graphWriteRepository.upsertState(
                workspaceId, newGeneration, OffsetDateTime.now(), "PASS");
            graphWriteRepository.purgeGeneration(workspaceId, currentGeneration);
          });
      log.info(
          "rebuild PASS for workspace {}: generation {} -> {}",
          workspaceId,
          currentGeneration,
          newGeneration);
    } else {
      TransactionTemplate failTx = requiresNew(projectorTransactionManager);
      failTx.executeWithoutResult(
          status -> {
            projectorRlsBinder.bindCurrentTransaction(workspaceId);
            graphWriteRepository.purgeGeneration(
                workspaceId, newGeneration); // drop the divergent N+1
            graphWriteRepository.upsertState(
                workspaceId, currentGeneration, OffsetDateTime.now(), "FAIL");
            graphWriteRepository.insertGap(
                divergenceAlert(workspaceId, newGeneration, equivalence));
          });
      log.error(
          "rebuild FAIL for workspace {}: generation {} stays live, {} dropped — {} node type "
              + "mismatch(es), {} edge type mismatch(es)",
          workspaceId,
          currentGeneration,
          newGeneration,
          equivalence.nodeMismatches().size(),
          equivalence.edgeMismatches().size());
    }
   ...
   * runs in, per T007's dual-datasource design) as the divergence alert record, per this task's own
   * suggested "log + a projection_gap-style record" framing. {@code event_type =
   ...
   */
  private ProjectionGap divergenceAlert(
      UUID workspaceId,
    ...
    return new ProjectionGap(
        ...
        mismatchTypes.toArray(new String[0]),
        OffsetDateTime.now(),
  ...
  private void buildArtifactRevisionNodes(
      UUID workspaceId, int generation, List<GraphNode> nodes, List<GraphEdge> edges) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
    ...
    for (Map<String, Object> row : rows) {
      ...
      NodeEdgeMapper.GateEventFact fact =
          new NodeEdgeMapper.GateEventFact(
              workspaceId,
              gateId,
              (String) row.get("gate_type"),
              (String) row.get("gate_definition_key"),
              ((Number) row.get("gate_definition_version")).intValue(),
              (UUID) row.get("case_instance_id"),
              (UUID) row.get("subject_artifact_revision_id"),
              "REBUILD_SNAPSHOT",
              null,
              null,
              gateId,
  ...
  private void buildTraceLinkNodes(
      UUID workspaceId, int generation, List<GraphNode> nodes, List<GraphEdge> edges) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
  ...
  /**
   * Phase 5 US3 (T021) — see {@link Projector}'s javadoc's "dependency/DEPENDS_ON — wired, still
   * writer-less" section.
   */
  ...
  /**
   * Phase 6 US4 (T025) — see {@link Projector}'s javadoc's
   * "knowledge_injection_snapshot/INJECTED_INTO" section.
   */
  private void buildInjectionSnapshotNodes(
      UUID workspaceId, int generation, List<GraphNode> nodes, List<GraphEdge> edges) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
