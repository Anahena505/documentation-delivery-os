 ...
 * ARTIFACT_REVISION}/{@code REQUIREMENT}/{@code OPERATION_EXECUTION}/{@code PRODUCED}
 * (artifact_revision scan), {@code GATE}/{@code GATED_BY} (gate events), {@code TRACES_TO}/{@code
 * DERIVES_FROM}/{@code SATISFIES} (trace_link scan), and — as of Phase 5 US3 (T021, {@link
 * com.d2os.projection.cycle.CycleDetector}) — {@code DEPENDS_ON} (dependency scan). Deliberately
 * DEFERRED, not wired in this phase: {@code PACKAGE}/{@code execution_package} (tasks.md T009's own
 * "minimum" scope for the rebuild-equivalence pair names only {@code case_instance}, {@code
 * artifact_revision}, {@code gate_instance}, {@code trace_link}); {@code
 * KNOWLEDGE_ITEM_VERSION}/{@code INJECTED_INTO} (explicitly assigned to Phase 6/US4 by tasks.md
 * T025 — "ensure the projector materializes INJECTED_INTO edges..." implies it is NOT yet wired,
 * until this same phase wires it below); {@code DEFINITION_VERSION} (no task in Phase 3-6 names a
 * concrete source table for it).
 *
 ...
 * application path wrote {@code dependency} rows (verified then, still true now — this task did not
 * add one; that remains a separate, out-of-scope gap). {@link
 * com.d2os.projection.cycle.CycleDetector} (T021) needs {@code DEPENDS_ON} edges to exist to have
 * anything to check, so this phase wires the scan on anyway: whenever something DOES write a {@code
 * dependency} row (today, only integration tests via direct SQL — see {@code CycleDetectionIT}),
 * this sweep picks it up like any other source table, exactly as {@link #scanTraceLinks} already
 * does for {@code trace_link}. {@link RebuildJob} and {@link EquivalenceVerifier} are wired
 * identically in this same phase so a rebuild does not silently drop the type and equivalence does
 * not spuriously flag it (see this class's own reasoning below on keeping the three in lockstep).
 ...
 * this only changes what counts as {@code source_ref} — FR-003's "no unsourced element" invariant
 * is unaffected: {@code source_ref} still always resolves to a real, dereferenceable row.
 *
...
public class Projector {
  ...
  public Projector(
      ...
      @Qualifier("projectorTransactionManager")
          PlatformTransactionManager projectorTransactionManager,
  ...
  @Scheduled(
      fixedDelayString = "${d2os.projection.consumer-interval-ms:5000}",
        ...
        () -> {
          ...
          for (UUID workspaceId : workspaceIds) {
            ...
            try {
              ...
              // Metrics are best-effort — a failed observation never affects projection
              // correctness.
              log.debug(
                  "projection metrics observation failed for workspace {}: {}",
                  workspaceId,
                  e.toString());
            }
          }
          // T020: publish the sweep's aggregate to the gauges (d2os.projection.lag.seconds /
          // .gap.open).
  ...
  /**
   * T020 — computes this workspace's projection lag and OPEN-gap count for {@link
   * ProjectionMetrics}, inside a short RLS-bound read transaction (both source tables are
   * RLS-scoped). These are exactly the values {@code GraphAdminController#status} exposes per
   * workspace: lag = seconds since the oldest still-unconsumed outbox row, open gaps = {@code
   * projection_gap} rows with {@code status='OPEN'}.
   */
  ...
  /**
   * Package-visible for {@link RebuildEquivalenceIT}-style callers that want to drive one workspace
   * directly.
   */
  void processWorkspace(UUID workspaceId) {
    ...
    List<GraphEdge> newDependsOnEdges =
        batch.edges().stream()
  ...
  private record ReadBatch(
      List<GraphNode> nodes,
      List<GraphEdge> edges,
      List<ProjectionGap> gaps,
      long watermarkBefore,
      long newWatermark) {
    boolean isNoOp() {
          ...
          if (!audit.missingFields().isEmpty()) {
            gaps.add(
                new ProjectionGap(
                    ...
                    eventId,
                    eventType,
                    audit.missingFields().toArray(new String[0]),
                    OffsetDateTime.now(),
  ...
  private void mapCaseEvent(
    ...
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList("SELECT feature_id FROM case_instance WHERE id = ?", caseId);
    if (rows.isEmpty()) {
    ...
    NodeEdgeMapper.CaseLifecycleFact fact =
        new NodeEdgeMapper.CaseLifecycleFact(
  ...
  private void mapGateEvent(
    ...
    try {
      ...
      return; // structurally required fields already validated by the auditor; malformed JSON is
      // defensive-only
    }
    ...
    NodeEdgeMapper.GateEventFact fact =
        new NodeEdgeMapper.GateEventFact(
            workspaceId,
            gateId,
            gateType,
            gateDefinitionKey,
  ...
  private void scanTraceLinks(
      UUID workspaceId, int generation, List<GraphNode> nodes, List<GraphEdge> edges) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
    ...
    for (Map<String, Object> row : rows) {
      NodeEdgeMapper.TraceLinkFact fact =
          new NodeEdgeMapper.TraceLinkFact(
              workspaceId,
              (UUID) row.get("id"),
              (String) row.get("from_type"),
              (UUID) row.get("from_id"),
              (String) row.get("to_type"),
              (UUID) row.get("to_id"),
              (String) row.get("link_type"),
  ...
  /**
   * Phase 5 US3 (T021) — see class javadoc's "dependency/DEPENDS_ON — wired, still writer-less"
   * section.
   */
  private void scanDependencies(
    ...
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
            ...
            workspaceId);
    for (Map<String, Object> row : rows) {
      NodeEdgeMapper.DependencyFact fact =
          new NodeEdgeMapper.DependencyFact(
              workspaceId,
              (UUID) row.get("id"),
              (String) row.get("from_type"),
              (UUID) row.get("from_id"),
  ...
  /**
   * Phase 6 US4 (T025) — see class javadoc's "knowledge_injection_snapshot/INJECTED_INTO — wired"
   * section.
   */
  private void scanInjectionSnapshots(
      UUID workspaceId, int generation, List<GraphNode> nodes, List<GraphEdge> edges) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
            ...
            workspaceId);
    for (Map<String, Object> row : rows) {
      NodeEdgeMapper.InjectionSnapshotFact fact =
          new NodeEdgeMapper.InjectionSnapshotFact(
              workspaceId,
              (UUID) row.get("id"),
              (UUID) row.get("operation_execution_id"),
              (UUID) row.get("knowledge_item_id"),
              (String) row.get("knowledge_item_key"),
  ...
  private void scanArtifactRevisions(
      UUID workspaceId, int generation, List<GraphNode> nodes, List<GraphEdge> edges) {
    List<Map<String, Object>> rows =
        jdbcTemplate.queryForList(
    ...
    for (Map<String, Object> row : rows) {
      ...
      NodeEdgeMapper.ArtifactRevisionFact fact =
          new NodeEdgeMapper.ArtifactRevisionFact(
              workspaceId,
              (UUID) row.get("artifact_id"),
              revisionId,
              ((Number) row.get("revision_no")).intValue(),
              (String) row.get("artifact_type"),
              (UUID) row.get("produced_by_operation_execution_id"),
              (UUID) row.get("source_template_id"),
              (String) row.get("template_version"),
              revisionId,
              toOffsetDateTime(row.get("created_at")));
