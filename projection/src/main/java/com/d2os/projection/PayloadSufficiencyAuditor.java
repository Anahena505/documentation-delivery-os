 ...
 * sets (research R6, FR-011). Only the {@code gate_instance} events are given a real field contract
 * in Phase 3 — those are the ONLY ones whose payload the {@link Projector}/{@link NodeEdgeMapper}
 * actually reads to shape a node/edge (the Phase 5 {@code GateEventPayload} contract this projector
 * consumes). Case-lifecycle events ({@code Planned}/{@code Running}/...) are mapped by re-reading
 * {@code case_instance} directly (see {@link Projector}'s javadoc) rather than by parsing the
 * outbox payload, so there is no payload field for them to be "thin" on — declaring an (empty)
 * contract for them here would be theater, not a real check. Every OTHER {@code event_type}
 * enumerated by grepping {@code auditWriter.record(} call sites across the repo ({@code
 * CASE_TYPE_CONFIRMED}/{@code CASE_TYPE_OVERRIDDEN}, {@code ARTIFACT_WRITE_REFUSED}, {@code
 * DEFINITION_PUBLISHED}, {@code GATE_APPROVE}/{@code GATE_REJECT}/{@code GATE_REQUEST_CHANGES}, the
 * knowledge-module events) is simply not a type the Phase 3 projector maps at all — {@link
 * #auditGateEvent} returns {@link AuditResult#notDeclared()} for anything outside {@code
 * gate_instance}'s three real event types, and the {@link Projector} never calls this auditor for
 * those either. This IS a scoped-down decision, documented honestly rather than inventing
 * hypothetical requirements for event types nothing downstream projects yet.
 *
 ...
 * event is skipped, a gap recorded); a content-thin event is STILL projected (the missing attribute
 * just comes through null, exactly as {@link NodeEdgeMapper#mapGateEvent} already tolerates), with
 * a gap recorded for the missing content so it surfaces via {@code GET /graph/admin/gaps} rather
...
public class PayloadSufficiencyAuditor {
  ...
  private static final Map<String, FieldContract> GATE_EVENT_CONTRACTS =
      Map.of(
          "GATE_OPENED",
              new FieldContract(
   ...
   *
   * @param declared false when this event type carries no contract in Phase 3 (not audited at all —
   *     see class javadoc)
   * @param projectable false only when a STRUCTURAL field is missing — the event cannot be mapped
   *     at all and the {@link Projector} must skip it entirely
   * @param missingFields every missing field (structural + content), for the {@link ProjectionGap}
   *     row's {@code missing_fields} column
   */
  ...
  public PayloadSufficiencyAuditor(
   ...
   * read-side transaction against the app datasource is a separate connection entirely, per T007's
   * dual-datasource design); the caller is responsible for having already decided this event needs
   * a gap row (i.e. {@link AuditResult#missingFields()} is non-empty).
   */
  public void recordGap(
      UUID workspaceId, UUID eventId, String eventType, List<String> missingFields) {
  ...
  public void alertIfPastThreshold(UUID workspaceId) {
    ...
    if (openCount >= gapAlertThreshold) {
      log.warn(
          "projection sufficiency: workspace {} has {} OPEN projection_gap rows (threshold {}) — "
   ...
   * watermark — WITHOUT re-projecting anything, purely for retroactive gap analysis. Existing OPEN
   * gaps for an event are not duplicated (checked by {@code event_id} before inserting).
   */
  ...
  /**
   * Count of previously-recorded gap event ids, so a re-run of the dry sweep does not duplicate
   * rows.
   */
  private List<UUID> existingGapEventIds(UUID workspaceId) {
    ...
    return readTx.execute(
        status -> {
          workspaceRlsBinder.bindCurrentTransaction(workspaceId);
          return jdbcTemplate.queryForList(
              "SELECT event_id FROM projection_gap WHERE workspace_id = ?",
              UUID.class,
              workspaceId);
        });
  }
